...
public static class ColorTemperatureConverter
{
    ...
    {
        var actualKelvin = ClampKelvin(kelvin, minKelvin, maxKelvin);
        var temperature = actualKelvin / 100f;

        var red = temperature <= 66f ? 255f : 329.698727446f * MathF.Pow(temperature - 60f, -0.1332047592f);
        var green = temperature <= 66f
            ? 99.4708025861f * MathF.Log(temperature) - 161.1195681661f
            : 288.1221695283f * MathF.Pow(temperature - 60f, -0.0755148492f);
        var blue = temperature >= 66f ? 255f : temperature <= 19f ? 0f : 138.5177312231f * MathF.Log(temperature - 10f) - 305.0447927307f;
