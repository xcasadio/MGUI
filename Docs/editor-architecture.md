...
La selection ne prend jamais le focus clavier. Elle ne peut pas non plus le garder : `MGWindow.ActivatesOnClick` (vrai par defaut) fait resoudre une nouvelle cible de focus a tout clic qui n'en pose pas lui-meme, ce qu'un clic sur la preview non interactive ne fait jamais. Le volet texte est bien une cible de navigation legitime depuis [decisions/0014-keyboard-focus-navigation-single-rule.md](decisions/0014-keyboard-focus-navigation-single-rule.md), mais ce n'est pas lui que cette resolution retient : son repli « premier element focusable » deprioritise les `ITextEntryHost`. Le volet texte active donc `Caret.ShowWhenUnfocused` : le caret que la selection deplace reste visible, sans clignoter, la ou elle l'a pose. Un clic sur une entree de la liste de diagnostics, lui, prend le focus (il pose son propre focus, donc l'activation ne le deplace pas), et c'est voulu : c'est un geste d'edition, pas de selection.
...
- Pour taper apres avoir selectionne depuis la preview ou l'arbre, il faut cliquer dans le volet texte, et ce clic deplace le caret : `MGTextBox` prend le focus et positionne le caret dans le meme geste, sans distinguer le premier clic. Le caret reste visible entre-temps, mais la position que la selection a posee est perdue des qu'on reclique.
