...
| `SCN-ANIM-003` | animation V3 | un easing de Bezier CSS anime un bouton en XAML, un etat nomme `OverridesLocalValue` l'emporte sur une valeur locale la ou un etat plain ne l'emporte pas, un toggle a un vrai slot `Checked` code-seul distinct du theme, un clip JSON multi-pistes (`Opacity`, `RenderTransform.Scale`, `Background`) se joue sur une cible et se previsualise sans jamais s'enregistrer aupres du manager (slider `Seek`, `Detach`), un storyboard se serialise puis se recharge depuis son JSON, le texte revele et la brush de surbrillance suivent la pause et le `TimeScale` de l'horloge du desktop | `MGUI.Samples/Features/AnimationDemo.xaml` (colonne V3) | `FullyQualifiedName~Animation\|FullyQualifiedName~AnimationDemo` |
...
- `SCN-ANIM-003` : [animation-architecture.md](animation-architecture.md), [decisions/0008-animation-v3-editor-readiness-and-remaining-limits.md](decisions/0008-animation-v3-editor-readiness-and-remaining-limits.md), taches [Tasks/animation-v3-tasks.md](Tasks/animation-v3-tasks.md).
