...
- Modes `Progress` / `Scan` de `MGHighlightBorderBrush` : parcours du perimetre rectangulaire. Les modes `Pulse` et `Flash` passent par le chemin shape-aware (`MGHighlightBorderBrush.Draw(..., MGBoxShape, MGBoxGeometry)`).
