...
- Chaque paint porte le contrat gelable (`IUIFreezable` : `IsFrozen`, `CanFreeze`, `Freeze()`) et une comparaison de valeur ; `Copy()` rend toujours une copie non gelee. Aujourd'hui (ADR-0009) ce sont des membres d'interface par defaut sur `IFillBrush` / `IBorderBrush` pour les brushes pas encore converties (une brush struct est toujours gelee, `ValueEquals` delegue a `Equals`) ; la garde de valeur de `MGControlTemplate` passe par `UIBrushEquality.ForGuards<T>()` plutot que par `EqualityComparer<T>.Default` directement (identite non pertinente pour un defaut de theme) ; depuis le fix round W2-fix, les setters de slot de `VisualStateSetting<T>` passent par `UIBrushEquality.ForSlots<T>()`, qui compare par IDENTITE pour un `T` brush : une instance distincte est un changement meme de meme valeur, pour qu'une mutation ulterieure de cette instance (brush possedee par l'element, W4) atteigne l'element. Depuis W2, `MGSolidFillBrush` et les brushes de gradient (`MGGradientFillBrush`, `MGDiagonalGradientFillBrush`) sont des classes mutables gelables derivant de `UIFreezableBrush`, avec proprietes settables notifiantes et `ValueEquals`/`Equals` propres ; les autres brushes de valeur et les composites restent a convertir (cible V4, voir Docs/animation-architecture.md « Brushes animables »).
