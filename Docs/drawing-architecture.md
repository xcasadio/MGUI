...
- Les hooks d'update restent cote paint : `IBorderBrush.Update(UpdateBaseArgs)` et `IFillBrush.Update(UpdateBaseArgs)` existent avec une implementation par defaut vide ; `MGHighlightBorderBrush.Update` ne fait plus tourner sa propre animation (ADR-0009) -- il transfere seulement l'appel a son `Underlay`, `AnimationProgress` etant desormais ecrit par un run possede par l'`MGElement` dont ce brush est la bordure effective (`SyncBorderHighlightRun`, voir Docs/animation-architecture.md, « Brushes animables »).
- Chaque paint porte le contrat gelable (`IUIFreezable` : `IsFrozen`, `CanFreeze`, `Freeze()`) et une comparaison de valeur ; `Copy()` rend toujours une copie non gelee. Les dix brushes de valeur (ADR-0009 : `MGSolidFillBrush`, `MGGradientFillBrush`, `MGDiagonalGradientFillBrush`, `MGTextureFillBrush`, `MGNineSliceFillBrush`, `MGProgressBarGradientBrush`, `MGUniformBorderBrush`, `MGDockedBorderBrush`, `MGBandedBorderBrush`, `MGTexturedBorderBrush`) sont des classes mutables gelables derivant de `UIFreezableBrush`, avec proprietes settables notifiantes et `ValueEquals`/`Equals`/`GetHashCode` propres ; les six composites/surbrillance (`MGCompositedFillBrush`, `MGPaddedFillBrush`, `MGBorderedFillBrush`, `MGCompositedBorderBrush`, `MGHighlightFillBrush`, `MGHighlightBorderBrush`) adoptent aussi le contrat, `Freeze` propageant aux enfants qu'ils possedent (voir ci-dessous). `IUIFreezable` reste porte par defaut sur `IFillBrush` / `IBorderBrush` pour compatibilite binaire ; `MGHighlightFillBrush` et `MGHighlightBorderBrush` s'appuient encore sur le `ValueEquals` par defaut (comparaison par reference), toutes les autres brushes du noyau surchargent `ValueEquals`. La garde de valeur de `MGControlTemplate` passe par `UIBrushEquality.ForGuards<T>()` plutot que par `EqualityComparer<T>.Default` directement (identite non pertinente pour un defaut de theme) ; les setters de slot de `VisualStateSetting<T>` passent par `UIBrushEquality.ForSlots<T>()`, qui compare par IDENTITE pour un `T` brush : une instance distincte est un changement meme de meme valeur, pour qu'une mutation ulterieure de cette instance (brush possedee par l'element) atteigne l'element.
- Les paints composites transferent les appels de cycle de vie a leurs enfants : `MGBorderedFillBrush` (fill et border), `MGCompositedFillBrush`, `MGPaddedFillBrush`, `VisualStateFillBrush` (etats dedupliques par reference, car les constructeurs a un seul brush partagent l'instance entre etats), `MGUniformBorderBrush`, `MGDockedBorderBrush` (cotes dedupliques par reference), `MGBandedBorderBrush`, `MGCompositedBorderBrush`, `MGHighlightBorderBrush`. `Freeze()` suit la meme propagation pour ces memes paints composites (recursif, deduplique par reference la ou `Update` l'est deja) : `CanFreeze` est faux si un enfant refuse de geler, et la liste `Brushes` de `MGCompositedFillBrush`/`MGCompositedBorderBrush` refuse `Add`/`Insert`/`Remove`/`Clear`/l'indexeur en ecriture une fois le composite gele. `VisualStateFillBrush.Copy()` (le conteneur lui-meme n'est jamais gele, seul son contenu peut l'etre) partage par reference un slot deja gele (`NormalValue`, `SelectedValue`, `FocusedValue`, `DisabledValue`, `CheckedValue`) et ne copie profondement qu'un slot non gele.
- Regles de partage (ADR-0009) : geler = partager, ne pas geler = posseder et observer. Une brush d'un theme construit (`ThemeDefinitionBuilder.Build`, `MGTheme.FreezeBrushes()`) ou d'une `StaticResource` (`MGResources.AddStaticResource`/`SetStaticResource`) est gelee et remise par reference a chaque consommateur (deux elements du meme type resolvent la meme instance de theme, une `StaticResource` est partagee sans copie) ; `MGTheme.Copy()` partage ces brushes gelees au lieu de les recopier. Une brush inline XAML ou creee par code reste non gelee et appartient a l'element qui la recoit ; `VisualStateFillBrush` s'abonne au `PropertyChanged` d'un slot non gele (jamais d'un slot gele, desabonnement au remplacement du slot ou au gel differe du slot par le passage de gel d'un theme/d'une ressource) et `MGBorder` fait de meme pour une `BorderBrush` non gelee, relayant la mutation comme un changement de conteneur/de bordure sans reecrire le store de valeurs resolues (le slot n'a pas change de reference) : muter une brush non gelee en place suffit a changer ce que le prochain dessin peint, MGUI ne mettant jamais un dessin en cache.
...
- Fills textures : `MGTextureFillBrush` projette ses UV sur `Geometry.Vertices` (chemin `DrawTexturedTriangleList`). Regles conservees dans le paint : `Fill` et `UniformToFill` projettent depuis la destination etiree ; `Uniform` et `None` projettent de la meme facon sous un clip rectangulaire exprime en espace ecran (la partie non couverte de la forme reste vide, comme sur le chemin rectangle) ; `Tile` sur la texture entiere tuile par sampler Wrap avec des UV en unites de tuile ; `Tile` sur un sous-rectangle d'atlas (un sampler Wrap repeterait tout l'atlas) tuile par quads textures : meme grille de tuiles et meme clampage du rectangle source pour une tuile partielle de bord que le chemin rectangle, mais chaque quad de tuile est decoupe (Sutherland-Hodgman, `MGConvexPolygonClipper.ClipToConvexPolygon`) contre `Geometry.OuterContour`, ses UV viennent de `MGConvexPolygonClipper.InterpolateRectUV` (rectangle destination de la tuile vers le rectangle source, possiblement clampe l'un et l'autre), sampler clamp (pas de wrap), les quads sont triangules en eventail et regroupes en aussi peu d'appels `DrawTexturedTriangleList` que le permet `short.MaxValue` sommets.
  - Frames de planche de sprites (ADR-0011, decision C4) : `FrameGrid`/`FrameIndex` remplacent le rectangle source effectif et la taille naturelle des trois chemins (rectangle etire ou en mosaique, arrondi) sans les reecrire. Rectangle source effectif : sans grille, `Source.SourceRect` inchange (y compris `null`, transmis tel quel a `DrawTextureTo` sur le chemin rectangle non tuile) ; avec une grille, `MGSpriteSheetGrid.GetFrameRectangle(FrameIndex, region)` ou `region` est `Source.SourceRect` (ou l'image entiere sans lui). Taille naturelle : sans grille, `Source.RenderSize` inchange ; avec une grille, `Source.RenderSizeOverride` s'il est pose, sinon la taille de cellule de la grille -- cette taille alimente `GetStretchedDestination` (`None`/`Uniform`/`UniformToFill`/`Fill`) et le pas de tuile des deux chemins tuiles (la tuile partielle de bord reste clampee depuis le coin haut-gauche de la cellule, meme regle que sans grille). Cote arrondi, la cellule d'un frame est toujours un sous-rectangle strict de l'atlas, meme quand la grille couvre l'image entiere (`Columns * Rows == 1` mis a part) : le test `isWholeTexture` du chemin `Tile` echoue donc systematiquement des qu'une grille est posee, et le rendu passe par la mosaique en quads decoupes (jamais le sampler `LinearWrap` sur l'image entiere, qui montrerait les frames voisines).
- Bordures texturees : `MGTexturedBorderBrush` conserve exactement la disposition du chemin rectangle (texture de coin sur les blocs de coin de la taille de l'epaisseur, textures de bord sur les rectangles de bord entre ces blocs) et laisse l'anneau arrondi couper cette disposition : chaque quad de `BorderRingIndices` est decoupe (Sutherland-Hodgman) contre chaque rectangle de la disposition, les morceaux sont triangules en eventail et emis par region (au plus 8 appels), de sorte que les coutures suivent les lignes de la disposition et non les quads. La partie de l'anneau plus profonde que l'epaisseur dans le carre d'un coin (quand le rayon depasse l'epaisseur) n'appartient a aucun rectangle de la disposition : elle est decoupee a part et peinte avec la texture de coin en coordonnees clampees, donc l'anneau n'a jamais de trou et les morceaux couvrent exactement son aire. Rotations et reflexions de `TextureTransforms` sont appliquees autour du centre de chaque rectangle avec la meme taille ajustee que le chemin rectangle (UV toujours dans [0,1], y compris pour un angle libre). Quand une epaisseur atteint (ou depasse) le rayon d'un coin, `MGBoxGeometryBuilder` ne laisse plus l'arc interne de ce coin s'effondrer a un point isole : le contour interne repete ce point autant de fois que le contour externe a emis de points pour ce coin (triangles d'anneau d'aire nulle a cet endroit), donc `HasBorderRingMesh` reste vrai et l'anneau se dessine normalement. Cas restant : quand l'epaisseur consomme entierement la boite (`InnerBounds` vide), il n'y a toujours aucun contour interne ; le paint garde alors le chemin rectangle, commente dans le code.
- Nine-slice texture : `MGNineSliceFillBrush` garde les 9 rectangles de destination du chemin rectangle (decoupes depuis `Shape.OuterBounds` par `TargetMargin`), mais decoupe chacun (Sutherland-Hodgman, `MGConvexPolygonClipper.ClipToConvexPolygon`) contre `Geometry.OuterContour` avant de le dessiner : un patch qui tombe sous un coin arrondi est coupe a la silhouette au lieu de deborder. Les UV de chaque patch viennent de son propre rectangle source (`MGConvexPolygonClipper.InterpolateRectUV` applique au rectangle de destination), donc un pixel sur un bord droit non coupe recoit exactement l'UV que le chemin rectangle aurait echantillonne la. Les patchs qui partagent une image et un masque de couleur sont regroupes dans un seul appel `DrawTexturedTriangleList`, scinde seulement si un lot depasserait `short.MaxValue` sommets.
- `MGHighlightFillBrush` : garde les memes rectangles focus/unfocus que le chemin rectangle (union de `FocusedBounds`/`FocusedElements`, soustraction mise en cache par `Bounds`), mais decoupe chacun (`MGConvexPolygonClipper.ClipToConvexPolygon`) contre `Geometry.OuterContour` avant de le peindre (eventail de `FillTriangle`) : un rectangle focus ou unfocus qui deborde d'un coin arrondi est coupe a la silhouette au lieu de peindre par-dessus l'arrondi. Un rectangle entierement interieur ressort inchange, dans le meme ordre de sommets.
- Modes `Progress` / `Scan` de `MGHighlightBorderBrush` (Tache 4 item 5) : `Progress` parametre le point central de la bande par longueur d'arc cumulee le long de `Geometry.OuterContour` / `Geometry.InnerContour` au lieu du perimetre rectangulaire ; le pourcentage 0 est ancre au sommet du contour le plus haut puis le plus a gauche (fin de l'arc du coin haut-gauche / debut du bord du haut, meme position que le coin haut-gauche du chemin rectangle quand le rayon est nul), le balayage part vers la droite le long du bord du haut pour `HighlightFlowDirection.Clockwise` (sens inverse pour `CounterClockwise`), les quads d'anneau entierement dans la bande sont emis entiers, ceux a cheval sur une frontiere sont decoupes en interpolant les deux contours a la longueur d'arc de la frontiere, et une bande qui boucle au-dela de la fin du contour est emise en deux morceaux. `Scan` calcule le rectangle de bande exactement comme le chemin rectangle, puis decoupe chaque quad d'anneau contre cette bande (`MGConvexPolygonClipper.ClipToRectangle`) et peint ce qui reste. Les deux modes dessinent par `FillTriangle`, comme `DrawBorderRing` et les modes `Pulse`/`Flash`. Repli conserve uniquement pour le chemin rectangle (`UsesRectangleFastPath`) et le cas residuel sans anneau (Tache 4 item 7, `InnerBounds` vide).

### Encore lies au rectangle (par conception, localise dans le paint)

- Aucun repli restant a ce jour : chaque paint avance consomme desormais `Geometry` et ne retombe sur le chemin rectangle que pour `UsesRectangleFastPath` ou le cas residuel sans anneau (Tache 4 item 7), les deux commentes a l'endroit du repli.

Ces replis residuels sont volontairement localises dans les implementations de paint, commentes a l'endroit du repli, et ne fuient jamais dans `MGBorder`, `MGRectangle` ni le builder de geometrie.
...
- Cycle de vie des fill brushes : sont tickes une fois par frame les paints atteints par `MGElement.Update`, c'est-a-dire `BackgroundBrush` (tous etats, dedupliques par reference), `OverlayBrush`, les surcharges de `GetFillBrushes()` / `GetVisualStateFillBrushes()` (`MGSlider`, `MGRectangle`, `MGShapeElementBase`, `MGScrollViewer`, `MGProgressButton`, `MGProgressBar`, `MGGridColorPicker`, `MGOverlayHost`, `MGUniformGrid`, `MGGrid`, `MGGridSplitter`) et les border brushes de `GetBorderBrushes()`. Ne sont volontairement pas surcharges : les proprietes qui redirigent vers le `BackgroundBrush` d'un enfant, deja ticke par cet enfant (`MGSpoiler.UnspoiledBackgroundBrush`, `MGTabControl.HeaderAreaBackground`, `MGExpander.ExpanderButtonBackgroundBrush`, `MGToggleButton.CheckedBackgroundBrush`) ; les brushes modeles consommes par le `BackgroundBrush` d'autres elements, dont l'appel `Update` transite par chaque consommateur : depuis ADR-0005/S5 (l'element detenteur s'abonne au `PropertyChanged` de son conteneur, donc un conteneur partage a longue duree de vie enracinerait tous ses detenteurs), `MGDockAutoHideStrip.ButtonBackgroundBrush` et `MGTreeView.SelectionBackgroundBrush` sont recopies (`Copy()`) dans chaque bouton de la bande et dans HeaderPanel, HeaderContainer et l'expandeur de l'item selectionne, chaque copie etant tickee par son detenteur et le modele lui-meme jamais ; `MGListBox.AlternatingRowBackgrounds` reste assigne par reference au slot `NormalValue` de chaque ligne, avec l'effet deduplique par frame au niveau du desktop, voir regle generale ci-dessous ; les brushes d'etat dont seule la valeur courante est recopiee, donc tickes uniquement tant qu'ils sont l'etat courant (`MGDockSplitterBar` et `MGDockTabItem` Normal/Hover/Pressed-Active) ; et les brushes de `MGGraphView` dont seule la couleur est extraite. Regle generale : `MGDesktop.Update` reinitialise un registre de dedup par frame (par reference d'objet, `IPaintUpdateRegistry`/`PaintUpdateRegistry`) porte par `UpdateBaseArgs.PaintRegistry`, et chaque tick de paint (`IFillBrush.Update` / `IBorderBrush.Update`, y compris a travers les paints composites et `VisualStateFillBrush`) passe par `PaintLifecycle` qui consulte ce registre ; un paint stateful partage par reference entre plusieurs emplacements ou elements avance donc une seule fois par frame au total, quel que soit le nombre de references, dans le meme element ou dans des elements differents, y compris pour un border brush partage.- Paints encore lies au rectangle (voir la matrice ci-dessus), chacun avec un repli commente dans le code et un item en Tache 4 : `MGHighlightBorderBrush` (`DrawProgressOnRing` / `DrawScanOnRing`) et `MGTexturedBorderBrush` (avec `DrawBorderRing` / `DrawTexturedBorderRing`) dans le cas residuel ou l'anneau de bordure est vide parce que l'epaisseur consomme entierement la boite (`InnerBounds` vide, donc pas de contour interne) ; `MGBoxGeometryBuilder.BuildBorderRingIndices` exige toujours des contours de meme taille, mais depuis la Tache 4 item 7 le contour interne egale systematiquement le contour externe des qu'il existe (un coin dont l'epaisseur atteint le rayon repete son point au lieu de raccourcir le contour), donc ce repli ne se declenche plus quand un simple coin s'effondre. Depuis la Tache 4 item 5, les modes `Progress` / `Scan` de `MGHighlightBorderBrush` parametrent l'animation le long du contour arrondi des que l'anneau existe (voir "Etat des paints avances" ci-dessus) ; seul ce meme cas residuel les fait encore retomber sur le perimetre rectangulaire.
...
Le fichier de suivi `Docs/Tasks/drawing-tasks.md` a ete retire une fois tous ses items livres ; les limites residuelles sont celles de la section Limites connues ci-dessus.
