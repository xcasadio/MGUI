...
- Chaque paint porte le contrat gelable (`IUIFreezable` : `IsFrozen`, `CanFreeze`, `Freeze()`) et une comparaison de valeur ; `Copy()` rend toujours une copie non gelee. Aujourd'hui (ADR-0009, W1) ce sont des membres d'interface par defaut sur `IFillBrush` / `IBorderBrush` (une brush struct est toujours gelee, `ValueEquals` delegue a `Equals`) ; les gardes de valeur (`VisualStateSetting<T>`, `MGControlTemplate`) passent par `UIBrushEquality.ForGuards<T>()` plutot que par `EqualityComparer<T>.Default` directement, pour rester correctes quand une brush deviendra une classe mutable derivant de `UIFreezableBrush` (cible V4, voir Docs/animation-architecture.md « Brushes animables »).
