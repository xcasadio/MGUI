...
- Les hooks d'update restent cote paint : `IBorderBrush.Update(UpdateBaseArgs)` existe avec une implementation par defaut vide ; `MGHighlightBorderBrush.Update` fait tourner son animation et transfere l'appel a son `Underlay`. `IFillBrush` n'a PAS de hook `Update`.
- Les paints composites doivent transferer les appels de cycle de vie de facon centralisee et previsible. Une lacune connue subsiste : voir Limites connues.
...
- `MGBorderedFillBrush` rejette `MGHighlightBorderBrush` (`NotImplementedException`, `MGUI.Core/UI/Brushes/Fill Brushes/MGBorderedFillBrush.cs:33-36`) car `IFillBrush` n'a pas de methode `Update` : l'animation ne serait jamais mise a jour. Le principe « les paints composites transferent les appels de cycle de vie » n'est pas encore tenu sur ce chemin.
...
- Le hit testing reste rectangulaire : `MGBoxShape` n'a aucun membre `Contains` / hit-test ; les surfaces interactives arrondies sont testees sur leurs bounds.
