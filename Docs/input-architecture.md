...
Par ailleurs : `MGDesktop.ActiveWindow` (`MGDesktop.cs:995`, `FocusedKeyboardHandler?.SelfOrParentWindow`) est une notion de "fenetre active" purement observationnelle, derivee du focus clavier — rien ne rappelle `BringToFront` automatiquement en reaction a un changement de focus — et `MGDesktop.ActiveWindowChanged` (`MGDesktop.cs:999`) s'invoque quand elle change. Par defaut, `MGWindow.ActivatesOnClick = true` cable en plus un clic dans une fenetre a `BringToFront` + un deplacement du focus resolu par `MGDesktop.ResolveAutoFocusTarget` (voir "Activation au clic derriere un overlay modal" plus haut) ; `MGFloatingDockWindow` s'appuie desormais sur ce meme mecanisme generique plutot que sur son propre cablage. `AllowsClickThrough`, un overlay modal actif, ou `ActivatesOnClick = false` (defaut sur `MGContextMenu`, `MGToolTip`, le dropdown de `MGComboBox`, la popup du color picker) restent des cas ou le clic ne ramene pas la fenetre au premier plan, donc focus et z-order peuvent toujours diverger silencieusement dans ces cas.
...
- **Eligibilite effective** (centralisee dans `MGDesktop` via `FocusInputPolicy`, `MGUI.Core/UI/FocusInputPolicy.cs`) : il faut `CanHandleKeyboardInput`, un hote capable de recevoir le clavier ce frame, l'absence de blocage par overlay modal, et un etat visible/interactif. Le desktop nettoie activement le focus courant **et** la file de focus quand une cible devient invalide en cours de tick ; un overlay modal actif force le nettoyage immediat d'un focus visant le contenu derriere lui.
- **Une seule regle de cible de navigation** (ADR-0013). `MGDesktop.IsNavigationTarget` teste `CanHandleKeyboardInput` — et non `IsFocusable`, dont le test y etait redondant, puisque `CanHandleKeyboardInput` vaut `IsFocusable` par defaut. `IsFocusable = true` reste donc l'opt-in normal d'un controle non textuel, mais un controle qui **surcharge** `CanHandleKeyboardInput` (`MGTextBox`, `MGMenuBar`) n'est plus exclu de la navigation faute d'avoir pose le drapeau. Consequence directe : les controles de saisie — `MGTextBox` et ses derives `MGPasswordBox`, `MGRichTextBox`, `MGNumericUpDown` — sont des arrets de tabulation, restaurables comme dernier element focus d'une fenetre et eligibles comme `DefaultFocusElement`, editeurs internes des composites compris (`MGPropertyGrid`, `MGChatBox`, `MGXAMLDesigner`, boite de commentaire de `MGGraphControls`), y compris en lecture seule.
- **Depriorisation des `ITextEntryHost` dans la resolution d'auto-focus.** `UIFocusNavigationService.GetFirstAutoFocusCandidate` ne choisit une text box comme *premier focusable* que s'il n'existe aucun autre candidat. Ce repli est le seul creneau concerne : `MGWindow.DefaultFocusElement`, l'historique de focus de la fenetre et l'ordre de tabulation ne sont pas filtres. Motif : un `ITextEntryHost` editable focus rend `MGDesktop.ShouldCaptureGameplayInput()` vrai, donc un simple clic dans le corps vide d'une fenetre (`MGWindow.ActivatesOnClick`) ne doit pas avaler l'input gameplay tant qu'un autre controle peut prendre le focus. La resolution a **une seule implementation** : `MGDesktop.ResolveAutoFocusTarget(MGElement, bool)` delegue au service.
...
- **Touches preservees** : les touches revendiquees par un `ITextEntryHost` focus (`MGUI.Core/UI/ITextEntryHost.cs` ; `MGTextBox.ShouldPreserveTextEntryKey` : fleches, Home/End, Space si editable, Enter/Tab selon `AcceptsReturn`/`AcceptsTab`, expose en implementation d'interface explicite) ne sont jamais reinterpretees en navigation — **sur les deux chemins** : garde dans `UIFocusNavigationService.TryDispatchNavigationAction(BaseKeyPressedEventArgs)` pour le chemin brut, et dans la surcharge semantique `TryDispatchNavigationAction(UINavigationAction, KeyboardFocusSource, Keys?)` alimentee par `MGDesktop.TryHandleInputAction`, qui lui transmet `actionEvent.Context.Key`. Les deux chemins testent `focusedElement is ITextEntryHost` (`UIFocusNavigationService.cs:216,454`), pas le type concret `MGTextBox` — un type tiers qui implemente l'interface beneficie de la meme protection.
- **`AcceptsTab` vaut `false` par defaut** (comme WPF) depuis ADR-0013 : sans cela, le Tab atteindrait une text box devenue arret de tabulation sans jamais pouvoir en ressortir. Les controles qui consomment Tab a bon droit le posent eux-memes : `MGRichTextBox` (`:143`), la boite de commentaire de `MGGraphControls` (`:3478`), la boite de markup editable de `MGXAMLDesigner`.
- **Ctrl+Tab / Ctrl+Shift+Tab sortent quand meme** d'un controle qui reserve Tab (`FocusInputPolicy.IsTextEntryNavigationEscape`, convention WPF), sinon ces controles resteraient des pieges clavier : les fleches et Home/End sont reservees **inconditionnellement**, et Echap se mappe sur `Cancel` que le repli de navigation ne traite pas. La regle ne libere **que** Tab : Ctrl+Gauche et Ctrl+Droite gardent leur sens d'edition. Aucune garde n'a ete ajoutee dans `MGTextBox` pour empecher l'insertion : la navigation brute est dispatchee depuis le `HighPriorityKeyboardHandler` du desktop, avant toute fenetre, et marque l'evenement handled — `KeyboardHandler` ne livre alors plus `Pressed` a la text box.
...
- `MGUIInputContext(MGDesktop desktop, int priority = 100)` : **reserve inconditionnellement toutes les actions UI** (meme si aucun controle ne les consomme finalement) et bloque les actions gameplay quand `MGDesktop.ShouldCaptureGameplayInput()` est vrai — overlay modal actif, menu contextuel ouvert, ou `ITextEntryHost` editable focus (`MGDesktop.cs:558`, `FocusedKeyboardHandler is ITextEntryHost focusedTextEntryHost && !focusedTextEntryHost.IsReadonly`).
...
- **Un seul weak event sanctionne** : `MGResources.SetParent` s'abonne au `OnDefaultThemeChanged` et au `OnStaticResourceLookupChanged` du parent via `WeakParentScopeForwarder` (classe imbriquee de `MGUI.Core/UI/MGResources.cs`, anciennement `WeakThemeChangedForwarder` ; le second evenement a ete ajoute le 7 septembre 2026, voir ADR-0001), qui ne reference le scope enfant que par `WeakReference` et s'auto-desabonne quand l'enfant est collecte. C'est le seul point d'abonnement croisant deux domaines de duree de vie ; la propagation de theme et de ressources fonctionne fenetre fermee comprise (donc a la reouverture), et le domaine complet devient collectable quand l'application lache l'instance. Ce n'est pas un feu vert pour des weak events generalises.
...
1. **`MGWindow.WindowKeyboardHandler` reste un point d'extension inerte par construction, desormais `[Obsolete]`, resolu par `MGWindow.PreviewKeyboardHandler`.** Son proprietaire est la fenetre elle-meme : `MGElement` surcharge explicitement `IKeyboardHandlerHost.HasKeyboardFocus()` (`MGUI.Core/UI/MGElement.cs:1621`, `GetDesktop().FocusedKeyboardHandler == this`), qui n'est vrai que si la fenetre **elle-meme** est l'element focus — ce qui n'arrive jamais, donc `Pressed` ne peut jamais lui etre livre. Le membre est conserve tel quel (compatibilite source pour un abonnement existant) mais marque `[Obsolete]`. Le point d'extension window-scoped reellement livre est `MGWindow.PreviewKeyboardHandler` : pompe **avant** les enfants de la fenetre — l'equivalent window-scoped du `HighPriorityKeyboardHandler` du desktop, pas un fallback de non-consommation — un abonne voit donc chaque touche du tick que le focus clavier soit sur la fenetre ou sur n'importe quel descendant. Cela fonctionne parce que son proprietaire (`WindowPreviewKeyboardHandlerHost`, `MGUI.Core/UI/WindowPreviewKeyboardHandlerHost.cs:15`) n'est pas un `MGElement` : il conserve l'implementation par defaut de `IKeyboardHandlerHost.HasKeyboardFocus()` (`=> true`, `MGUI.Shared/Input/Keyboard/KeyboardTracker.cs:17`) au lieu de la surcharge de `MGElement` ci-dessus. Un abonne qui ne doit pas voler les touches de saisie de texte doit verifier `MGDesktop.FocusedKeyboardHandler` contre `ITextEntryHost` avant d'agir.
...
5. **La sortie clavier Ctrl+Tab n'existe que sur le chemin brut.** `InputActionContext` (`MGUI.Shared/Input/Semantic/InputActionContext.cs:23-31`) ne porte aucun modificateur, donc `MGDesktop.TryHandleInputAction` ne peut pas distinguer Ctrl+Tab de Tab : sur le chemin semantique, un controle qui reserve Tab reste un piege clavier. Le chemin brut est le defaut (`MGDesktop.UseRawNavigationInput = true`) et seuls `MGUI.MiniGame` et l'outillage de replay utilisent l'autre. Etendre ce `record struct` public est un chantier separe (ADR-0013).
6. Divers : hit-test rectangle-only (coins arrondis ; `MGElement.RenderTransform` et `RenderScale` sont inverses par le hit-test (ADR-0006), `ToLocalUnscaledPoint` dans `IsInside` et `ComputeTopmostHoveredElement`, gate par `MGDesktop.ActiveRenderTransformCount` : la zone cliquable suit les pixels dessines) ; transitions sous-frame perdues (press+release dans une meme frame = aucun evenement, inherent au polling) ; `DerivedIsHitTestVisible` ne consulte que le parent immediat (le pipeline interne, lui, chaine correctement) ; `MGButton` ne marque pas ses clics handled par defaut ; `KeyboardHandler` itere ~160 `Keys` par handler par tick sans early-out agrege ; machinerie de priorite des trackers inutilisee par le framework lui-meme ; `FocusArchitectureTests` lit les sources via des chemins absolus.

## Reste a faire

Les 11 taches de l'ancien plan input, puis les 5 slices d'activation de
fenetre et de points d'extension (`ActiveWindow`, `MGWindow.ActivatesOnClick`,
`ITextEntryHost` + ses deux migrations, `MGWindow.PreviewKeyboardHandler`),
sont livrees (fichiers de taches retires). Le dossier de conception de ce
second lot reste consultable dans `Docs/input-window-activation-design.md`.
Aucun travail n'est ouvert sur ce theme a ce jour.
