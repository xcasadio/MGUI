...
Question centrale du routage : quand deux fenetres se superposent, la fenetre du dessus consomme-t-elle les evenements de sorte que la fenetre du dessous n'en recoive aucun ? Reponse courte : **oui pour les clics, la molette et les debuts de drag — non pour le survol (hover), l'etat presse visuel, et le clavier qui est route par focus et non par z-order.** Detail dans la section "Fenetres superposees".
...
1. **Le hover traverse.** Les evenements de mouvement n'ont pas de flag handled et le hit-test est purement par-fenetre, sans test d'occlusion inter-fenetres. La fenetre du dessous continue de calculer son `HoveredElement`, ses elements recoivent `Entered`/`MovedInside`/`Exited` et allument l'etat visuel `Hovered` (les deux fenetres surlignent en meme temps). Seuls les **tooltips** sont proteges, via la boucle manuelle `IsWindowOccludedAtMousePos` de `MGDesktop.Update`. `DragEnter/DragOver/DragLeave` (drag-drop) reposant sur ces memes evenements, ils traversent aussi.
2. **L'etat presse fuit.** `MGWindow.OnBeginUpdate` fixe `PressedElement` des que le tracker signale un press recent, **sans consulter le flag handled** : un clic dans la zone de recouvrement met l'element des **deux** fenetres en etat visuel `Pressed` et declenche `PressedElementChanged` dans les deux.
...
1. **Fuite de hover inter-fenetres** : etat `Hovered`, `Entered/MovedInside/Exited` et drag-drop atteignent les elements occlus d'une fenetre inferieure ; `PressedElement` est fixe sans consulter le flag handled (les deux fenetres s'allument) ; seuls les tooltips sont proteges via `IsWindowOccludedAtMousePos`.
2. **Inversion de z-order dans les replis de navigation** : `GetWindowsFrontToBack` (`MGDesktop.cs`, copie morte) et ses jumeaux vivants dans `UIFocusNavigationService` omettent le `.Reverse()` que la boucle d'update applique — quand rien n'est focus, Tab/auto-focus peut cibler la fenetre la plus en arriere.
3. **`MGWindow.WindowKeyboardHandler` est un point d'extension inerte** : son proprietaire (la fenetre) n'a jamais le focus clavier, donc `Pressed` ne peut jamais lui etre livre.
4. **Light-dismiss fragile hors du premier plan** : le dropdown `MGComboBox` se ferme sur `ReleasedOutside`, mais si le clic de fermeture atterrit sur une fenetre plus haute, la cloture de celle-ci consomme le release et le dropdown reste ouvert. Le menu contextuel n'y echappe que par son slot d'update privilegie.
5. **Le chemin semantique contourne la preservation des touches de saisie** : `MGDesktop.TryHandleInputAction` ne consulte pas `ShouldPreserveTextEntryKey` — avec `UseRawNavigationInput = false`, une fleche ou Tab pendant l'edition d'un textbox peut a la fois editer le caret et deplacer le focus.
6. **Fast-path drag mort** : les flags `MouseTracker.HasCurrentDragStart/Dragged/DragEndEvents` testent `x.Value != null` sur des dictionnaires internes jamais nuls et valent donc toujours `true` — le bloc de dispatch drag tourne chaque tick pour chaque handler abonne au drag (cout pur, pas de bug fonctionnel).
7. **Collectabilite des fenetres fermees** : la chaine de retention via l'abonnement theme de `MGResources.SetParent` est neutralisee par le forwarder faible ; un travail complementaire sur les racines residuelles `MGResources` des fenetres fermees est en cours dans une session dediee — ne pas re-specifier ce chantier ici.
8. **Aucune reaction a la desactivation de la fenetre OS** (`Game.IsActive` ignore).
9. Divers : hit-test rectangle-only (coins arrondis, `RenderScale` — la zone cliquable diverge des pixels dessines) ; transitions sous-frame perdues (press+release dans une meme frame = aucun evenement, inherent au polling) ; `DerivedIsHitTestVisible` ne consulte que le parent immediat (le pipeline interne, lui, chaine correctement) ; `MGButton` ne marque pas ses clics handled par defaut ; `KeyboardHandler` itere ~160 `Keys` par handler par tick sans early-out agrege ; machinerie de priorite des trackers inutilisee par le framework lui-meme ; `FocusArchitectureTests` lit les sources via des chemins absolus ; aucun test n'existe pour l'input de deux fenetres desktop superposees.
