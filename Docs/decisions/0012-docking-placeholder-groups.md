...
- T5, 2026-09-17: a 2.0 document whose `rootNode` is absent loads as an empty layout, because that is exactly what `ToJson` writes for a model with no root. A truncated file that loses its `rootNode` key therefore loads instead of being refused. Distinguishing the two would require always writing `rootNode`, even null, and rejecting a document without the key, which changes the round trip of an empty layout; left as an open point.
- T5, 2026-09-17: every node read from a document must carry a non-empty `id`; a missing one is a diagnostic, not an exception, so `TryFromJson` and `TryLoadLayoutFromJson` keep their "never throws" contract (D7).
