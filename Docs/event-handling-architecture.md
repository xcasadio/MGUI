...
**Fuite residuelle decouverte lors de la regression de bout en bout (tache 6, 2026-09, non couverte par D1-D4)** : fermer une fenetre (`MGWindow.TryCloseWindow` + retrait de `Desktop.Windows`) ne suffit pas a la rendre collectable. `MGWindow` cree en permanence, des son constructeur, un scope de ressources local (`EnsureResourceScope(UIResourceScope.Window)`) dont le `MGResources` s'abonne a `Desktop.Resources.OnDefaultThemeChanged` via `MGResources.SetParent`/`Parent_OnDefaultThemeChanged`, sans jamais s'en desabonner — chaine de retention verifiee experimentalement : `Desktop.Resources -> (delegate cible) window.LocalResources -> (event backing field) lambda capturant `this` -> window (et tout son sous-arbre)`. Volontairement **non corrige** ici : comme pour la fuite 4 (voir la note "Approche INTERDITE" ci-dessous), ce repo re-affiche couramment la MEME instance de fenetre apres fermeture (`SampleBase.Show/Hide`, `BringToFront`) ; desabonner ce lien au moment de la fermeture casserait silencieusement la propagation de theme a la reouverture, faute d'un point ou le rattacher. Corriger correctement exigerait une nouvelle decision de conception (ou/quand rattacher `MGResources.Parent` a la reouverture), hors perimetre D1-D4. Pinnee par `MGUI.Tests/Input/InputLifetimeRegressionTests.cs` (`WindowResourcesParentChain_IsAKnownResidualRoot_NotFixedByThisPlan` documente le chemin de retention ; `ClosingWindow_AllowsTextBoxNumericUpDownAndListBox_ToBeCollected_OnceKnownResidualRootIsDetached` prouve que les fuites 4/5 (D1-D3) sont bien corrigees une fois cette racine residuelle neutralisee).
