...
- Contrat : `IFillBrush` et `IBorderBrush` portent `IUIFreezable` (`IsFrozen`, `CanFreeze`, `Freeze()`) et `ValueEquals(other)` comme membres d'interface par defaut (ADR-0009, W1) : une brush encore `readonly struct` aujourd'hui est toujours consideree gelee (elle ne peut pas muter en place) et son `ValueEquals` delegue a `Equals`. L'abstraction `UIFreezableBrush` (`ViewModelBase`) fournit deja le socle qu'une brush concrete adoptera : proprietes settables qui verifient le gel (`ThrowIfFrozen`, `InvalidOperationException` sur une brush gelee), comparent, ecrivent et notifient sans allocation ; `Freeze()` gele recursivement les enfants via `OnFreeze`, `CanFreeze` est faux si un enfant ne peut pas l'etre ; `Copy()` doit rendre une copie profonde non gelee (l'equivalent de `Clone` WPF). (cible V4) Aucune brush concrete n'en derive encore : la conversion des dix brushes de valeur (`MGSolidFillBrush` avec sa palette statique `SolidFillBrushes`, `MGGradientFillBrush`, `MGDiagonalGradientFillBrush`, `MGTextureFillBrush`, `MGNineSliceFillBrush`, `MGProgressBarGradientBrush`, `MGUniformBorderBrush`, `MGDockedBorderBrush`, `MGBandedBorderBrush`, `MGTexturedBorderBrush`) et des six classes existantes (les composites `MGCompositedFillBrush`, `MGPaddedFillBrush`, `MGBorderedFillBrush`, `MGCompositedBorderBrush`, `MGHighlightFillBrush`, `MGHighlightBorderBrush`) reste a livrer. Le conteneur `VisualStateFillBrush` n'est pas une brush : il reste copie par element, ses slots partageront les brushes gelees.
- Egalite : `UIBrushEquality.ValueEquals` (champ a champ, recursif) et `UIBrushEquality.ForGuards<T>()` servent aux seules gardes qui comparent des valeurs : les setters de slot de `VisualStateSetting<T>` (une brush de meme valeur ne notifie pas) et la garde de re-application d'un defaut de template au changement de theme (`MGControlTemplate.ApplyTemplateValueCore`). Aujourd'hui `ValueEquals` retombe sur `Equals`, donc sur la comparaison structurelle des dix brushes struct et la comparaison par reference des six classes mutables ; (cible V4) une fois ces brushes converties en classes, `Equals` deviendra par reference partout comme pour toute classe, sans que ces deux gardes changent de comportement observable puisqu'elles passent deja par `UIBrushEquality`. Le store compare par reference (`ReferenceEqualityComparer.Instance`), inchange par ce chantier.
