...
Tests : `TemplateStructureRebuildThemeTests` (combo box et fenetre comparees aux memes controles construits sous `Dark`, valeurs du proprietaire gardees), `ThemeRefreshRegressionTests`. Limites : une valeur pilote declaree en attribut sur une part d'un template XAML est enregistree `LocalValue` et reportee comme une valeur de l'application si un autre template remplace la part (tache 15 de `Docs/Tasks/styling-theme-tasks.md`) ; un conteneur (fond, texte) ou une propriete hors store pose par l'application sur une part remplacee est perdu (verifie le 12 septembre 2026 : un `MGTabControl.HeaderAreaBackground` pose par l'application revient au fond du theme apres le passage a `Dark`).
