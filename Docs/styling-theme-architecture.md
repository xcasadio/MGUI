...
Items d'une structure reconstruite : `MGListBox`, `MGTabControl` et, depuis le 12 septembre 2026, `MGTreeView` replacent leurs items dans le panneau de la nouvelle structure (`AttachControlTemplateStructure`), qui les remet dans l'arbre visuel, la mise en page et la notification de theme ; `MGTreeView` les retire d'abord de l'ancien panneau. Limite verifiee le 12 septembre 2026 : `MGListView` ne le fait pas ; sous `Dark.ListView`, ses grilles de donnees et d'en-tete reconstruites n'ont ni ligne ni colonne et la liste s'affiche vide.
