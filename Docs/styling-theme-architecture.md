...
## Themes et ressources geles (ADR-0009, W4)

- Une brush d'un theme construit est gelee : `ThemeDefinitionBuilder.Build` gele en une seule passe (`MGTheme.FreezeBrushes()`, reflexion recursive sur les proprietes publiques de `MGTheme` et de ses groupes de reglages -- `Window`, `Graph`, `Docking`, ... -- plus `_Backgrounds`, prive) toute brush atteinte (`IFillBrush`/`IBorderBrush`) et chaque slot non nul d'un `VisualStateFillBrush` (le conteneur lui-meme n'est jamais gele, W3). Toute propriete brush du theme gele des l'affectation, sans exception : une propriete geree par un `ThemeManagedGetter<T>` (`SliderForeground`, `ScrollBarOuterBrush`, une entree de `_Backgrounds`, ...) gele la brush recue dans le setter `Value` ; une propriete brute d'un groupe de reglages (`Window.BorderBrush`, `Graph.EdgeBrush`, `TreeViewBorderBrush`, ...) gele la brush recue dans son propre setter (champ prive + appel a `MGTheme.FreezeThemeValue`), corrige lors d'une revue (ADR-0009 W4) apres qu'une premiere livraison laissait ces proprietes non gelees entre l'affectation et le prochain `FreezeBrushes`.
- `ThemeManagedGetter<T>.GetValue` rend l'instance gelee sans la cloner quand la valeur stockee est directement une brush ; un conteneur `VisualStateFillBrush`/`VisualStateColorBrush` reste clone a chaque lecture (W3 : ses slots geles sont partages par la copie, ses slots non geles sont copies). `MGTheme.Copy()` (`ApplyFrom`) partage de meme chaque brush gelee au lieu de la recopier (`ShareOrCopy`) ; une brush non gelee (cas residuel, ex. une brush liee a un controle vivant qui ne peut pas geler) est toujours copiee.
- Une brush enregistree comme ressource statique (`MGResources.AddStaticResource`, `SetStaticResource`, y compris le remplacement d'une `DynamicResource`) est gelee des l'enregistrement, pour que `UIResourceReferenceApplicator` continue de la remettre par reference a chaque consommateur en toute securite.
- Une brush inline XAML (`ToFillBrush`/`ToBorderBrush` d'un DTO) ou creee par code reste non gelee et appartient a l'element qui la recoit : la muter (`brush.Color = ...`) est le mecanisme normal d'une brush possedee par l'element (element unique, ou plusieurs elements partageant volontairement la meme instance non gelee). Voir `Docs/drawing-architecture.md`, "Cycle de vie des paints stateful", pour la notification (abonnement `PropertyChanged` cote `VisualStateFillBrush`/`MGBorder`, jamais pour une brush gelee).
...
- Une animation gagne pendant sa duree puis la valeur revient a la meilleure source non animee. Depuis la tranche S4 du programme animation (12 septembre 2026, ADR-0006), le niveau `Animation` (100) est alimente par les cibles du moteur (`MGUI.Core/UI/Animation/Targets/`) : Margin, Padding et MinHeight (S4), puis les slots de couleur unie du fond, du texte et de la bordure (S5), chaque tick ecrivant par le setter tague avec `UIValueResolutionSource.Animation(kind, nom)` ; l'arret restaure par `ClearPilotSource(..., Animation)`. Une animation terminee en `HoldEnd` garde sa contribution jusqu'a la prochaine animation du meme chemin ou `Animations.Clear()` : une ecriture locale posee pendant ce temps reste masquee et le diagnostic rapporte `Animation` (asymetrie assumee avec les proprietes hors store, qui gardent simplement leur derniere valeur).
...
Depuis la tache U9 (13 septembre 2026, ADR-0008 decision 9), le refresh re-transfere aussi les transitions et les etats visuels nommes des styles courants, dans le meme ordre que la parse (style implicite fusionne des ressources, puis styles inline, puis styles nommes, le dernier gagnant par chemin ou par nom). Chaque transition ou etat pose par un style porte une provenance (`UITransition.Provenance` / `UIVisualState.Provenance`, un `UIValueSourceKind?` : `ImplicitStyle`, `ExplicitStyle`, ou nul pour une declaration de l'element ou ajoutee par code) et une signature deterministe (`Signature`, construite depuis le chemin, la duree, le delai, l'identite de l'easing (`ToString()`, pas le type CLR : tous les easings nommes integres partagent la meme classe) pour une transition ; le nom, `OverridesLocalValue` et chaque setter pour un etat). Sur un refresh :

- une transition ou un etat de style absent de l'element est ajoute (compte dans `WrittenTransitions`/`WrittenVisualStates`) ;
- un existant a provenance non nulle dont la signature a change est remplace (une transition en cours s'arrete et garde sa valeur courante, comme `UITransitionCollection.Add` le fait deja ; un etat courant remplace restaure puis reapplique aussitot ses nouveaux setters, `UIVisualStateCollection.Add` ayant ete corrige pour cela) ; une signature inchangee ne touche a rien ;
- un existant a provenance non nulle que plus aucun style ne pose est retire (`ClearedTransitions`/`ClearedVisualStates` ; une transition retiree garde sa valeur en cours, un etat courant retire restaure sa base tout de suite, pas au prochain changement d'etat) ;
- un chemin ou un nom declare par l'element lui-meme (`<Element.Transitions>`/`<Element.VisualStates>`, capture a la parse dans `ElementStyleScope.OwnTransitionPaths`/`OwnVisualStateNames`) ou une entree a provenance nulle (ajoutee par code) n'est jamais touche : l'element et l'application l'emportent toujours sur un style.

Deux refresh sans changement n'ecrivent rien (les quatre compteurs a zero) et ne relancent jamais une transition en cours. `MGElement.RefreshedStyleTransitionPaths` / `RefreshedStyleVisualStateNames` gardent, pour le diagnostic, ce que le dernier refresh a laisse a la charge d'un style (nul avant le premier refresh, comme `RefreshedStyleProperties`). La passe des setters ci-dessus n'est pas modifiee ; le cout reste borne a `RefreshStyles`, sans surcout par frame.
...
- des groupes dedies pour les familles durables: `FontSettings`, `Window`, `Overlay`, `ContextMenu`, `ContextMenuItem`, `ListBox`, `ListView`, `PropertyGrid`, `ComboBox`, `TreeViewTemplate`, `TabControl`, `Graph`, `Docking`, et depuis la tache 14 `ToolTip`, `TextBox`, `NumericUpDown`;
...
### Densites pilotables par le theme (tache 14)

Depuis le 12 septembre 2026, les densites que le preset "Editor Compact" (tache 13) ne pouvait pas atteindre sont des reglages de theme, avec pour valeur par defaut la valeur que le code imposait :

- `ToolTip` (nouveau groupe `MGThemeToolTipSettings`) : `Padding` (6,3), `BorderThickness` (2), `BorderBrush` (noir), `MinWidth` et `MinHeight` (10) ; le decalage de dessin et la couleur du texte restent `ToolTipOffset` et `ToolTipTextForeground` ;
- `TextBox` (`MGThemeTextBoxSettings`) : `Padding` (6,1,6,1), `MinHeight` (24) ; `NumericUpDown` (`MGThemeNumericUpDownSettings`) : `Padding` (6,2,6,2), `MinHeight` (28), `SpinnerWidth` (24), `SpinnerMinWidth` (22) ;
- `ListBox.ItemPadding` (6,4) et `ListBox.ItemContentPadding` (1,0), dont les champs statiques `MGControlTemplateCatalog.DefaultListBoxItemPadding` et `DefaultListBoxItemContentPadding` restent les defauts ; `MGListBox.OnThemeChanged` re-applique les deux sur les items existants (le contenu par defaut porte la contribution `Template` nommee `ListBox.ItemContent.Padding`, un contenu fourni par l'application est laisse tel quel) et `GetThemeInvalidation` ne demande une mise en page que si l'une des deux change ; `ComboBox.DropdownItemPadding` (8,5,8,5, defaut `DefaultComboBoxDropdownItemPadding`) ;
- `TabControl.SelectedHeaderPadding` (8,5,8,5) et `UnselectedHeaderPadding` (8,3,8,3) pour des en-tetes en haut ou en bas, `SideHeaderPadding` (6,5,6,5) pour des en-tetes a gauche ou a droite ; les epaisseurs de bordure des en-tetes restent des valeurs de template, elles encodent le cote qui touche le contenu ;
- `Docking.TabHeaderHeight` (30, `MGDockTabGroup.TabHeaderHeight` et `MGDockTabItem.TabHeight` ; le groupe donne sa hauteur aux items qu'il construit et repousse un changement sur les items qui la suivaient encore, un item dont l'application a fixe la hauteur ne suit plus), `TabButtonSize` (22, `MGDockTabItem.ButtonSize`, largeur reservee aux boutons fermer et epingler), `TabTitlePadding` (8,4,4,4), `AutoHideDrawerHeaderHeight` (28, `MGDockAutoHideDrawer.HeaderHeight`), `AutoHideDrawerButtonSize` (22, `HeaderButtonSize`), `AutoHideStripThickness` (24, `MGDockAutoHideStrip.StripThickness`, devenu une propriete d'instance dont `DefaultStripThickness` garde la constante, et `MGDockHost.AutoHideStripThickness`, l'inset reserve par l'hote, donne aux strips qu'il attache et repousse sur celles qui le suivaient encore, selon la meme regle), `DropIndicatorZoneSize` (40, `MGDockDropIndicators.ZoneSize`, dont le setter recalcule les rectangles de zones et de bords d'hote). Le proprietaire (groupe, hote) applique sa valeur avant que ses items ou strips ne recoivent leur propre refresh de template : cette regle "suit tant qu'il etait egal" est ce qui protege une valeur posee par l'application sur un seul item ou une seule strip.

Les applicateurs du catalogue lisent ces reglages par `ApplyOwnerThemeDefault` (valeur du controle lui-meme, gardee par la garde has-previous/equals : une valeur posee par l'application n'est pas ecrasee par un changement de theme) ou `ApplyThemeDefault` (valeur d'une part). Le preset `EditorCompact` les declare toutes ; `EditorCompactPresetTests` verifie leur application a des controles vivants, aller et retour, et `DockTabItemVisualsTests` la propagation du groupe d'onglets vers ses items.
...
- pendant un refresh de theme (`IsThemeRefresh`), la valeur n'est re-appliquee que si la valeur courante correspond encore au dernier default applique (garde has-previous/equals), sauf une valeur de part apres reconstruction de la structure (voir plus bas) ; pour les proprietes pilotes, la precedence du store protege en plus une valeur locale, un style ou un binding quelle que soit la garde ;
...
Comportement runtime : un changement de theme declenche `OnThemeChanged` puis `ApplyControlTemplate(true)`. La structure n'est reconstruite que si le template de structure resolu change, par exemple quand le theme `Dark` mappe `Window`, `ToolTip`, `Overlay`, `ContextMenu`, `ListBox`, `ListView`, `ComboBox`, `TreeView` ou `TabControl` sur une variante `Dark.*` qui declare ses propres `DetachedRoots` ; sinon seul le chrome est re-applique.

Structure reconstruite pendant un refresh (12 septembre 2026). Les enregistrements de `_AppliedTemplateDefaults` sont indexes par nom sur le proprietaire, pas par cible : celui d'une valeur de part decrit l'ancienne part, alors que la nouvelle porte sa valeur de construction. Avant ce correctif, la garde comparait ces deux valeurs et sautait l'ecriture : une combo box et une fenetre construites sous `Dark_Blue` gardaient, apres le passage a `Dark`, le fond de liste deroulante, le chrome de barre de titre, le contenu du bouton de fermeture ou la largeur preferee de leur construction. `MGElement.ApplyControlTemplate` passe desormais `MGControlTemplateContext.IsStructureRebuilt` a l'application qui suit l'instanciation d'une structure :

- une valeur de part (source `Template` : `ApplyThemeDefault`, `ApplyTemplateValue`) est appliquee sans consulter son enregistrement ;
- une valeur du controle lui-meme (source `Theme` : `ApplyOwnerThemeDefault`) garde la garde has-previous/equals, pour ne pas ecraser une valeur posee par l'application ; la surcharge non taguee `ApplyOwnerThemeDefault(Name, Value, GetCurrentValue, Action<T> SetValue, Invalidation, Comparer)` porte les valeurs du controle hors store (`ComboBox.DropdownArrowColor`, `ToolTip.DrawOffset`, selection et indentation de `TreeView`, noms de templates d'en-tetes de `TabControl`, couleurs et alignements de `TextBox`, brosses de `GraphView`, couleurs docking) ;
- juste avant, les contributions qu'une part remplacee portait pour le proprietaire sont reportees sur la part de meme nom : `LocalValue`, `ImplicitStyle` et `ExplicitStyle` (facades `BorderBrush`/`BorderThickness`, epaisseur de bordure de `WindowStyle.None`), et les contributions `Theme` dont le proprietaire a enregistre la source (bordure d'une fenetre, d'une combo box, d'un `TabControl` ou d'une `TreeView`), afin que leur garde compare la valeur que le proprietaire tient encore. Tous les pilotes sont reportes : Margin, Padding, MinHeight, BorderBrush, BorderThickness, et les conteneurs de fond, de texte par defaut et de texte d'un `MGTextBlock` avec leurs sous-champs (fond pose par `MGTabControl.HeaderAreaBackground` sur le panneau d'en-tetes, couleurs de texte de la barre de titre). Un conteneur reporte garde son identite : la part remplacee abandonne sa contribution et revient a son propre conteneur, pour ne pas rester abonnee a celui qu'elle cede ;
- les initialisations faites une fois par structure testent `!IsThemeRefresh || IsStructureRebuilt` : contenu et alignements du bouton de fermeture de `Window` et `Overlay`, largeur preferee de la liste deroulante de `ComboBox`, glyphes de `NumericUpDown`, reglages du panneau d'en-tetes de `TabControl`. Celle de `ToolTip` reste limitee a la construction : elle regle des proprietes du tooltip lui-meme, que `MGWindow.AttachControlTemplateStructure` repousse sur la nouvelle structure.

Tests : `TemplateStructureRebuildThemeTests` (combo box et fenetre comparees aux memes controles construits sous `Dark`, valeurs du proprietaire gardees), `ThemeRefreshRegressionTests`. Limite : une propriete hors store posee par l'application sur une part remplacee (alignement, espacement, lignes de grille) est perdue.

Valeurs declarees par un template XAML (tache 15, 12 septembre 2026). Un attribut pilote pose sur un element d'un `ControlTemplate` XAML (`<Border Name="PART_Border" BorderThickness="3" />`, `<DockPanel Name="PART_TitleBar" Padding="9" Background="Red">`) est une valeur du template, pas de l'application : `ControlTemplateLoader.CreateTemplate` estampille la racine, les `DetachedRoots`, leurs descendants (`GetChildren`) et les elements que leurs proprietes tiennent hors des enfants (`Element.MarkAsTemplateStructure`, par reflexion sur les proprietes publiques typees `Element` ou listes d'elements et d'objets auxiliaires du namespace XAML : facade `Border` imbriquee des composites, en-tete d'un expander ou d'un onglet, parties de la liste deroulante d'une combo box, tooltip, menu contextuel, en-tete d'une `ListViewColumn`, ce dernier cas releve par la revue), et `Element.ResolveXamlSource` enregistre alors ces attributs en source `Template` nommee `"<template>:<nom d'element>.<propriete>"` (`Test.PaddedWindow:PART_TitleBar.Padding` ; la facade `Border` d'un bouton nomme prend le nom du bouton, un element sans nom celui de son type) au lieu de `LocalValue`. Regle entre cette declaration et la valeur que l'applicateur de la base pose sur la meme part (meme precedence `Template`, dernier ecrivain gagnant) : la declaration du template l'emporte. `ControlTemplateLoader.BuildStructure` conserve avec la structure les contributions `Template` ainsi nommees (`MGControlTemplateStructure.DeclaredValues`) et `MGElement.ApplyControlTemplate` les re-applique apres chaque `Template.Apply` : a la premiere application elle recouvre le defaut de l'applicateur ; a un refresh de theme, un defaut scalaire est deja saute par la garde has-previous/equals (la valeur courante n'est plus le defaut enregistre) tandis qu'un defaut conteneur (fond) la passe encore, remplace le conteneur et fait tomber les sous-champs declares de meme precedence (ADR-0005/S5), d'ou la re-application systematique. Le report d'une structure reconstruite ne reprend jamais une contribution `Template` : la part du nouveau template ne porte pas la declaration de l'ancien, et une structure reconstruite du meme template recoit la sienne. Un attribut pilote hors template garde sa provenance (`LocalValue`, ou style). Aucun des templates de `BuiltInControlTemplates.xaml` ne declare de valeur pilote ; `MGUI.Samples` non plus. Tests : `XamlTemplatePartValuesTests`. Les fonds et couleurs de texte sont reportes depuis le 12 septembre 2026 ; avant, un `MGTabControl.HeaderAreaBackground` pose par l'application revenait au fond du theme apres le passage a `Dark`.

Items d'une structure reconstruite : `MGListBox`, `MGTabControl` et, depuis le 12 septembre 2026, `MGTreeView` et `MGListView` replacent leurs items dans la nouvelle structure (`AttachControlTemplateStructure`), qui les remet dans l'arbre visuel, la mise en page et la notification de theme. `MGTreeView` retire d'abord ses items racines de l'ancien panneau. `MGListView` recree ses definitions de colonnes et de lignes dans les nouvelles grilles, puisqu'elles appartiennent a leur grille (`MGListViewColumn.HeaderColumn`/`DataColumn` et `MGListViewItem.DataRow` sont remplaces), y deplace en-tetes et cellules avec leurs elements, et reporte le mode et la cellule de selection ; les autres reglages hors store poses sur les grilles remplacees (lignes de grille, espacements, pinceaux de selection) sont perdus, comme toute propriete hors store d'une part.
...
- Refresh de styles a chaud limite aux pilotes (tache 10) : `MGElement.RefreshStyles()` ne re-applique que les proprietes dont le transfert XAML est une ecriture taguee du store (liste dans "Refresh de styles a chaud"). Un setter d'une autre propriete (alignement, taille, texte, ...) s'applique toujours a la parse, mais le refresh ne le re-applique ni ne le retire : sans store, rien ne distingue une valeur de style d'une valeur locale ; il est rapporte `NotRefreshable`. Les facades de bordure nommees autrement que `BorderBrush`/`BorderThickness` (`ListBox` Outer/Inner/Title, `Spoiler` Unspoiled*) ne sont pas rafraichies. Seuls les elements crees par une definition XAML dont les styles ont ete traites (parse avec ressources) ont un scope. Le refresh n'est jamais declenche automatiquement par `AddStyle`, `RemoveStyle` ou `AddImplicitStyle` : l'application l'appelle sur le sous-arbre voulu. Depuis la tache U9, les transitions et etats visuels des styles rejoignent ce refresh (voir "Refresh de styles a chaud") ; les transitions de theme (`UIThemeTransitions`) et tout ce que l'element ou le code declare directement restent hors de son perimetre par construction (provenance nulle).
- Preset declaratif "Editor Compact" (tache 13, `MGUI.Samples/Features/EditorCompact.Themes.xaml`) : un theme `BasedOn="Dark"` pilote par simple changement de theme les polices, le chrome de fenetre, l'overlay et le menu contextuel, la combo box, le titre de list box, la tree view, l'espacement des onglets, la property grid, la taille des cases a cocher et l'indentation d'arbre. Depuis la tache 14 (12 septembre 2026), les densites qu'il ne pouvait pas atteindre (tooltip, zones de texte et `NumericUpDown`, paddings d'items de list box et de liste deroulante, paddings des en-tetes d'onglets, tailles du docking) sont des reglages de theme (voir "Densites pilotables par le theme") et l'ecart `Dark.ListBox` est corrige. Restent hors de portee : les epaisseurs de bordure des en-tetes d'onglets (valeurs de template qui encodent le cote docke), les boutons compacts du groupe d'onglets docking (24, constantes privees), l'heuristique de hauteur minimale de `DockTabGroupNode` (30, modele sans acces au theme), les bandes de detection du drag (`DockDropCalculator.HostEdgeBandWidth` 40, `MGDockHost.ProximityBandWidth` 30), la taille des glyphes des zones de drop (24) et les tailles de boutons de la bande auto-hide (`ButtonMinSize` 60, `ButtonPadding` 20, police 11). Les definitions de `ControlTemplate` XAML ne portent aucune valeur de template : un theme peut seulement remapper un controle.
- Moteur de valeurs resolues limite aux sept pilotes (tache 4, ADR-0005) : Margin, Padding, MinHeight, BorderBrush, BorderThickness, Background, texte (huit cles, deux conteneurs pour le texte). Limites verifiees le 11 septembre 2026 : un conteneur (`VisualStateFillBrush`, `VisualStateSetting<Color?>`) partage entre plusieurs elements est enregistre par chaque detenteur et, l'element s'abonnant a son `PropertyChanged`, un conteneur partage a longue duree de vie enracine ses detenteurs ; le framework copie donc toujours les instances brutes du theme et les modeles de controle (`MGDockAutoHideStrip.ButtonBackgroundBrush`, `MGTreeView.SelectionBackgroundBrush`) avant de les remettre a un element, et une instance partagee par l'application reste sous sa responsabilite. Ceci reste vrai pour le CONTENEUR (`VisualStateFillBrush`, jamais gele) ; depuis ADR-0009/W4, le contenu qu'il porte (une brush de theme, une `StaticResource`) est gele et partage sans copie ni risque d'enracinement : une brush gelee ne notifie jamais, donc rien ne s'y abonne (voir "Themes et ressources geles" plus haut). Le niveau `Animation` est alimente par le moteur d'animation pour les pilotes de layout (S4) puis de couleur unie (S5) (voir "Arbitrages") ; hors selection d'arbre, noeud de graphe et onglets docking, `VisualState` n'est pas alimente par le framework. `_AppliedTemplateDefaults` et sa garde restent en parallele du store ; indexes par nom sur le proprietaire, ils ignorent quelle part une valeur visait, d'ou le contournement des valeurs de part et le report des contributions du proprietaire quand une structure est reconstruite (voir "Valeurs template et refresh"). Le diagnostic `TryGetResolvedPilotValue` d'un sous-slot de texte replie sur `Inherited`/`Theme` selon l'etat visuel courant, pas selon le slot demande. Les facades de bordure nommees autrement que `BorderBrush`/`BorderThickness` (`ListBox` Outer/Inner/Title/ItemsPanel, `Spoiler` Unspoiled*) rapportent `LocalValue` au lieu de la provenance de style. Un `Padding` declare en XAML sur une `Window` (attribut ou style) supplante l'ecriture `LocalValue` que le setter `WindowStyle` laisse au parse (le DTO la retire avant de re-appliquer le padding sous sa provenance) ; une affectation ulterieure de `WindowStyle` par l'application re-epingle Padding et BorderThickness en `LocalValue`, pas le fond (voir "Contrat de refresh de theme"). Dans la pile officielle, `DynamicResource` (30) est sous les styles : un setter de style sur la meme propriete l'emporte sur un attribut `{DynamicResource}` (aucun sample n'est dans ce cas).
