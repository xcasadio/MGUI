...
Tests : `TemplateStructureRebuildThemeTests` (combo box et fenetre comparees aux memes controles construits sous `Dark`, valeurs du proprietaire gardees), `ThemeRefreshRegressionTests`. Limites : une valeur pilote declaree en attribut sur une part d'un template XAML est enregistree `LocalValue` et reportee comme une valeur de l'application si un autre template remplace la part (tache 15 de `Docs/Tasks/styling-theme-tasks.md`) ; une propriete hors store posee par l'application sur une part remplacee (alignement, espacement, lignes de grille) est perdue. Les fonds et couleurs de texte sont reportes depuis le 12 septembre 2026 ; avant, un `MGTabControl.HeaderAreaBackground` pose par l'application revenait au fond du theme apres le passage a `Dark`.
