...
1. `Animation` (100)
2. `VisualStateOverride` (95) -- un etat nomme declare avec `OverridesLocalValue` (ADR-0008 decision 4) ; genre toujours `VisualState`
3. `LocalValue` (90)
4. `LocalBinding` (80)
5. `VisualState` (70)
6. `Template` (60)
...
- Une animation gagne pendant sa duree puis la valeur revient a la meilleure source non animee. Le niveau `Animation` (100) est alimente par les cibles du moteur d'animation (`MGUI.Core/UI/Animation/Targets/`, ADR-0006) : Margin, Padding et MinHeight, puis les slots de couleur unie du fond, du texte et de la bordure, chaque tick ecrivant par le setter tague avec `UIValueResolutionSource.Animation(kind, nom)` ; l'arret restaure par `ClearPilotSource(..., Animation)`. Une animation terminee en `HoldEnd` garde sa contribution jusqu'a la prochaine animation du meme chemin ou `Animations.Clear()` : une ecriture locale posee pendant ce temps reste masquee et le diagnostic rapporte `Animation` (asymetrie assumee avec les proprietes hors store, qui gardent simplement leur derniere valeur).
...
- une valeur du controle lui-meme (source `Theme` : `ApplyOwnerThemeDefault`) garde la garde has-previous/equals, pour ne pas ecraser une valeur posee par l'application ; la surcharge non taguee `ApplyOwnerThemeDefault(Name, Value, GetCurrentValue, Action<T> SetValue, Invalidation, Comparer)` porte les valeurs du controle hors store (`ComboBox.DropdownArrowColor`, `ToolTip.DrawOffset`, selection, indentation et taille de la colonne d'expansion de `TreeView`, noms de templates d'en-tetes de `TabControl`, couleurs et alignements de `TextBox`, brosses de `GraphView`, couleurs docking) ;
...
- Refresh de styles a chaud limite aux pilotes (tache 10) : `MGElement.RefreshStyles()` ne re-applique que les proprietes dont le transfert XAML est une ecriture taguee du store (liste dans "Refresh de styles a chaud"). Un setter d'une autre propriete (alignement, taille, texte, ...) s'applique toujours a la parse, mais le refresh ne le re-applique ni ne le retire : sans store, rien ne distingue une valeur de style d'une valeur locale ; il est rapporte `NotRefreshable`. Les facades de bordure nommees autrement que `BorderBrush`/`BorderThickness` (`ListBox` Outer/Inner/Title, `Spoiler` Unspoiled*) ne sont pas rafraichies. Seuls les elements crees par une definition XAML dont les styles ont ete traites (parse avec ressources) ont un scope. Le refresh n'est jamais declenche automatiquement par `AddStyle`, `RemoveStyle` ou `AddImplicitStyle` : l'application l'appelle sur le sous-arbre voulu. Les transitions et etats visuels des styles rejoignent ce refresh (voir "Refresh de styles a chaud") ; les transitions de theme (`UIThemeTransitions`) et tout ce que l'element ou le code declare directement restent hors de son perimetre par construction (provenance nulle).
