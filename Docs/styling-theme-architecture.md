...
Tests : `TemplateStructureRebuildThemeTests` (combo box et fenetre comparees aux memes controles construits sous `Dark`, valeurs du proprietaire gardees), `ThemeRefreshRegressionTests`. Limites : une valeur pilote declaree en attribut sur une part d'un template XAML est enregistree `LocalValue` et reportee comme une valeur de l'application si un autre template remplace la part ; un conteneur (fond, texte) ou une propriete hors store pose par l'application sur une part remplacee est perdu. Limite verifiee le 11 septembre 2026 et non traitee ici : `MGTreeView` recree son `ItemsPanel` en laissant les items existants parentes a l'ancien panneau.
