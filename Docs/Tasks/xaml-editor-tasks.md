...
- robustesse des cycles : un changement d'onglet, puis un cycle `DetachToFloating` et re-dock, laissent le contenu de l'hote intact : aucune exception, et l'hote n'affiche pas son contenu de repli d'erreur (les cinq volets sont toujours atteints depuis l'hote ou depuis la fenetre flottante) ;
...
- decision 7 (texte de `view.Root` et de `SetContent(view.Root)`), perimetre de X0 (description de la grille, « elements nommes ») et ce point ouvert : reecrits ou marques « remplace par X1b » ; le perimetre V1 passe a dix taches ;
