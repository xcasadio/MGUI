...
- Racine `Window` : le coin haut-gauche du volet est l'origine de la preview, et la fenetre s'y place au decalage que son document declare (`Left` et `Top`), comme elle se placerait depuis le coin d'un bureau. Regle revisee le 18 septembre a la demande de l'auteur : elle disait jusque-la que `Left` et `Top` ne placent pas la preview. Une `MGWindow` se mettant en page a sa propre position, l'hote repositionne la fenetre de preview sur le coin du volet apres chaque `PreviewUpdated` et a chaque changement des bounds du volet (deplacement de la fenetre de l'editeur, deplacement d'un separateur de docking, regroupement en onglet, redimensionnement), puis demande une mise en page. La preview garde `Width` et `Height` du XAML : elle n'est ni redimensionnee ni mise a l'echelle, et ce qui depasse est rogne par le clip du volet.
...
Statut (18 septembre 2026) : livree et verifiee (verifier en contexte frais : CONFIRMED au premier tour, avec ses propres sondes hors depot qui comparent le hit test d'outillage au vrai chemin d'entree ; suite complete 2844/2844). Retouches de la session principale apres verification : le test d'echelle construisait son point ecran avec la conversion meme que `HitTest` applique, donc il serait passe sans elle ; il est maintenant arithmetique (echelle autour du coin haut-gauche de la fenetre, `MGWindow.UpdateScaleTransforms`), garde le point hors des bounds non mis a l'echelle et echoue bien quand on retire la conversion (mutation verifiee) ; un troisieme cas de remontee a ete ajoute, le clic sur la barre de titre d'une racine `Window` previsualisee, les deux tests livres couvrant ensemble part de template et bouton interne du meme controle. Divergence assumee et tracee dans l'ADR : une selection venue du caret ne replace pas le caret, sinon elle ecrase la position posee par un clic dans la liste de diagnostics (X3). Reserve consignee : entre une frappe et la fin du debounce de la preview, l'adorner vise encore l'element de l'arbre precedent ; c'est borne a 250 ms et se corrige tout seul au re-parse.

Suite donnee a la validation manuelle de l'auteur (18 septembre) : il a valide les neuf cas, sauf que le caret pose par la selection etait invisible. Cause trouvee dans `MGUI.Core` : le caret n'est dessine que si le volet texte detient le focus clavier, et un clic dans la preview deplace justement le focus ailleurs, `MGWindow.ActivatesOnClick` resolvant une nouvelle cible a tout clic qui n'en pose pas lui-meme, cible que le volet texte n'etait alors jamais, puisque `MGDesktop.IsNavigationTarget` exigeait `IsFocusable` que `MGTextBox` ne posait pas (incoherence de framework corrigee depuis par un chantier a part, ADR-0014 : le volet texte est desormais une cible legitime, mais le repli « premier element focusable » deprioritise les `ITextEntryHost`, donc la conclusion tient toujours). Trou du plan : la section ci-dessous dit que selectionner place le caret, sans jamais dire qui detient le focus. Choix de l'auteur entre trois options : le caret reste visible sans focus et la selection ne prend jamais le focus. Livre : `MGTextCaret.ShowWhenUnfocused` (opt-in, faux par defaut) et `IsCurrentlyVisible`, tous deux adosses a une decision unique `ShouldDrawCaret` que `Draw` utilise aussi ; sans focus le caret ne clignote pas ; le volet texte de l'editeur active l'option. Verifier en contexte frais : CONFIRMED, aucune P0-P2 ; suite 2867/2867. Retouche de la session principale apres verification : les entrees de la decision etaient recopiees aux deux appels, elles sont passees en membres partages et `Draw` se lit desormais `if (IsCurrentlyVisible)`. Limite acceptee par l'auteur : pour taper il faut toujours cliquer dans le volet texte, et ce clic deplace le caret. Validee par l'auteur le 18 septembre 2026.
...
Statut (18 septembre 2026) : livree et verifiee (verifier en contexte frais : CONFIRMED, aucune P0-P2 ; suite complete 2889/2889). Faits qui gouvernent la conception, releves dans le code avant d'ecrire la tranche : `MGPropertyGridDescriptorCache.TryGetEditorKind` n'accepte que couleur, `bool`, entiers, `float`, `double` et `string`, et ecarte tout le reste SANS ERREUR, d'ou la regle du plan (chaque descriptor annonce `typeof(string)`, le vrai type restant cote source pour X6) ; le setter `MGPropertyGrid.SelectedObject` sort immediatement si l'instance est la meme et reconstruit des que la valeur est un `ICustomTypeDescriptor`, donc le cycle de vie du plan ne marche que si l'editeur le pilote ainsi. Ajout au plan : les lignes « Resolved (runtime) » sont nommees `Resolved.<chemin>` avec `DisplayName` = le chemin nu, parce que `Padding` existe des deux cotes et que deux descriptors de meme nom se masqueraient. Retouche de la session principale apres verification, sur deux constats reproduits hors depot par le verifier : la regle « meme selection » ne tenait qu'a l'ordinal, si bien qu'un noeud selectionne au caret avant le premier rendu ne gagnait jamais sa categorie « Resolved (runtime) », et qu'une balise reecrite au meme ordinal gardait les lignes du type precedent ; la regle exige desormais aussi le meme `DtoType` et la meme nullite du representant, ce qui reconstruit la grille dans ces deux cas sans la reconstruire pendant la frappe (deux tests ajoutes, prouves par mutation). Reste la validation manuelle de l'auteur.
...
- RESOLU le 18 septembre : le code d'un diagnostic ne depend plus de la langue de la machine (`4b51ca6`, branche `fix/xaml-diagnostics-culture`, arrive ici avec la fusion de `develop`) ; `Width="abc"` est classe `InvalidValueConversion` dans toutes les langues et X3 affiche le code tel quel.
