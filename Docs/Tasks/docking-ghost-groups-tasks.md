...
Statut (17 septembre 2026) : livre. Meme pipeline. Quatre constats confirmes : `RemovePanel(id)` sur un panneau masque laissait un bouton mort dans sa bande et le tiroir ouvert (corrige) ; `ShowDockable` laissait une place perimee quand il retombait sur le premier groupe visible (corrige) ; deux comportements corrects mais non testes (memoire de la place a la fermeture de la fenetre flottante, activation de l'onglet demande quand le panneau flotte) desormais couverts. Un P3 differe : la branche « deja dans le store auto-hide » de `ShowDockable` est inatteignable tant qu'un panneau masque reste dans `_panelRegistry` ; elle sert de garde apres chargement (T5). Mutations : M1 (fermeture sans memoire) → A4 rouges ; M2 (`ShowDockable` sans place memorisee) → A4 rouges ; M3 (branche flottante retiree) → duplicata detecte ; M4 (reouverture sans reconstruction) → A8 rouge ; plus quatre mutations ciblees pour les corrections ; toutes retirees. Validation : build 0 erreur ; suite complete 2523 reussis, 0 echec (2503 avant T4), verte deux fois. Note : l'executor a repare a la main des fins de ligne cassees par un `sed -i` lors d'une verification ; `MGDockHost.cs` est bien en CRLF et son diff reste cible (143 lignes ajoutees, 76 retirees).

Jalon V1 (17 septembre 2026) : **CONFIRMED** par un verifier en contexte frais sur le diff `4b51ca6..5db193b`. Suites relancees par lui : complete 2503/2503, docking 326/326, entree 258/258. Sondes hors depot au-dela des tests livres : 576 combinaisons (24 ordres de depart x 24 ordres de retour) melant flottement et masquage sur la disposition de l'editeur, toutes identiques a l'octet pres, sans fenetre flottante residuelle ; arbre entierement cache puis retour croise ; retour apres remplacement de la racine, sans fantome residuel ; dix cycles flotter/ancrer avec exactement deux `DockLayoutChanged` par cycle, sans derive d'abonnement ; bornes des fenetres flottantes ecrites dans le modele. Remarques hors perimetre du jalon : un meme id de panneau peut encore exister en double (garde prevue en T4 et T5), l'asymetrie `PanelAdded` / `PanelRemoved` au flottement est anterieure au chantier, et la branche « second enfant cache » n'a pas d'assertion dediee (exercee par les sondes).
