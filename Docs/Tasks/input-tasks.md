...
- Deux tests ajoutes a `MGUI.Tests/Focus/NavigationFrontToBackFallbackTests.cs`, un par site utilisant `.OrderByDescending(x => x.IsTopmost)` dans `UIFocusNavigationService.cs` :
  - `MoveFocusNext_WithIsTopmostWindow_AddedFirst_ResolvesToTopmostWindow_NotFirstAddedWindow` epingle `UIFocusNavigationService.cs:404` (`GetNavigationRoot`, `topWindowRoot`) : une troisieme fenetre `topmostWindow` (`IsTopmost = true`, bornes 500,300,200,200, loin des deux autres fenetres pour ecarter tout hover) est ajoutee EN PREMIER dans `Desktop.Windows`, avant `backWindow` et `frontWindow` (memes bornes que les 2 tests existants). Une asserte de sanite verifie que les 3 fenetres ont `HoveredElement == null` avant l'acte, pour que la resolution ne mesure que le repli sans hover. Puis `GetFocusableElements()` doit contenir uniquement `topmostButton` (et ni `frontButton` ni `backButton`), et `MoveFocusNext(KeyboardFocusSource.Keyboard)` doit focaliser `topmostButton`.
  - `MoveFocusNext_WithRealHoverOverOverlappingWindows_ResolvesToIsTopmostWindow` epingle `UIFocusNavigationService.cs:387` (`GetHoveredNavigationTarget`) : `topmostWindow` (`IsTopmost = true`, bornes 100,100,200,200) chevauche `overlappingWindow` (bornes 150,150,200,200, non-topmost). Pour isoler l'ordre de :387 du mecanisme d'occlusion inter-fenetres (tache 4), `topmostWindow` a `AllowsClickThrough = true` et son bouton (`topmostButton`) a `Opacity = 0.5f` : `FindFirstOpaqueParent` (`MGDesktop.cs:1440`) part du bouton (non opaque), remonte a la fenetre elle-meme (opaque par defaut), et le test `OpaqueHoveredElement != Window` a `MGDesktop.cs:1441` exclut ce cas — donc `overlappingWindow` n'est PAS occluse et calcule elle aussi un `HoveredElement` reel (`overlappingButton`) au meme point de la souris. Le test asserte explicitement que les deux fenetres ont un `HoveredElement` non nul (`topmostButton` et `overlappingButton`) et qu'aucune n'est `IsOccludedAtMousePos`, avant d'asserter que `MoveFocusNext(...)` focalise `topmostButton` — la resolution est donc imputable uniquement au tri de :387, jamais a une suppression par occlusion.
- Deviation par rapport au patron `desktop.Update(); desktop.Update();` des 2 tests existants (constatee en verifiant les asserts de sanite demandes par la tache, pas inventee a priori) : sans jamais appeler `runtime.ApplyFrame(...)`, `GraphTestRuntime.UpdateArgs` reste a sa valeur par defaut (`TimeSpan.Zero` ecoule) ; sur ce chemin, la fenetre qui se retrouve en tete du tri (`Windows.Reverse<MGWindow>().OrderByDescending(x => x.IsTopmost)`) affiche un `HoveredElement` non nul des le premier `desktop.Update()`, meme quand ses bornes reelles ne contiennent pas la position de la souris — verifie empiriquement (bornes {X:500 Y:300 W:200 H:200}, souris {X:0 Y:0}, `HoveredElement` neanmoins non nul), et reproduit aussi avec une fenetre non-topmost (c'est la fenetre en tete du tri qui est concernee, pas `IsTopmost` specifiquement). Cet artefact aurait rendu le test (A) non discriminant pour :404 (le `hoveredWindowRoot`, calcule via :387 non mute, aurait masque la mutation de :404 en resolvant quand meme vers la bonne fenetre par ce chemin parasite). Le test (A) utilise donc le helper `Frame(...)` (un `ApplyFrame` explicite avec un temps ecoule reel et une position de souris a (798, 598), a l'interieur de la surface 800x600 mais hors des trois fenetres) au lieu des `desktop.Update()` nus — cela fait disparaitre l'artefact (les 3 fenetres ont `HoveredElement == null` comme attendu) sans changer ce que le test verifie. Les 2 tests existants du fichier ne sont pas modifies (ils ne sont pas casses par cet artefact, ils resolvent juste la meme fenetre par un chemin possiblement different qui n'affecte pas leur assertion finale) ; aucune question a remonter, la deviation est documentee ici avec les faits empiriques qui la justifient.
- Verification par mutation (deux sites exacts, chacun revert individuellement) :
  - `UIFocusNavigationService.cs:404` : `.OrderByDescending(x => x.IsTopmost)` retire de `topWindowRoot` -> seul `MoveFocusNext_WithIsTopmostWindow_AddedFirst_ResolvesToTopmostWindow_NotFirstAddedWindow` rougit (`GetFocusableElements()` contient `frontButton` au lieu de `topmostButton`) ; les 3 autres tests du fichier restent verts. Mutation revertee, `git status --short -- MGUI.Core/UI/Navigation/UIFocusNavigationService.cs` confirme un arbre propre.
  - `UIFocusNavigationService.cs:387` : `.OrderByDescending(x => x.IsTopmost)` retire de la boucle de `GetHoveredNavigationTarget` -> seul `MoveFocusNext_WithRealHoverOverOverlappingWindows_ResolvesToIsTopmostWindow` rougit (`MoveFocusNext` focalise `overlappingButton` au lieu de `topmostButton`) ; les 3 autres tests du fichier restent verts. Mutation revertee, arbre propre reconfirme.
  - Mutation `MGDesktop.cs:1400` (boucle d'update, `OrderedWindows`) non testee : elle ne pilote ni :404 ni :387 (ce sont des expressions distinctes, verifie par lecture avant edition), donc ne peut pas exercer ces 2 tests ; c'est deja documente dans la tache 2 pour les 2 tests existants et reste vrai ici.
- Validation : `dotnet build MGUI.Tests/MGUI.Tests.csproj --no-restore` OK ; `dotnet build MGUI.Samples/MGUI.Samples.csproj --no-restore` OK ; `dotnet test MGUI.Tests/MGUI.Tests.csproj --no-build --filter "FullyQualifiedName~NavigationFrontToBackFallbackTests"` -> 4/4 verts ; `--filter "Focus|Input"` -> 417/417 verts (415 baseline + 2 nouveaux) ; suite complete `dotnet test MGUI.Tests/MGUI.Tests.csproj --no-build` -> 1427/1427 verts (1425 baseline + 2 nouveaux), aucun test nouvellement rouge.
