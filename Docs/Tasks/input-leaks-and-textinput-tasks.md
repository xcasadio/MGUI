...
- nouveau fichier `MGUI.Tests/Input/InputLifetimeRegressionTests.cs` (2 tests) : construit un `MGWindow` de test contenant un `MGTextBox`, un `MGNumericUpDown` et un `MGListBox<string>` dans un `MGStackPanel` ; exerce les 3 mecanismes cibles par D1-D3 (focus clavier successif sur chaque controle via le setter prive `FocusedKeyboardHandler`, press simule sur le listbox) ; ferme la fenetre via `TryCloseWindow()` et verifie son retrait de `Desktop.Windows` ; capture des `WeakReference` sur la fenetre + les 3 controles dans une methode `[MethodImpl(MethodImplOptions.NoInlining)]` separee ; boucle courte `GC.Collect()`/`WaitForPendingFinalizers()` (5 iterations) ;
- **racine residuelle decouverte** en ecrivant ce test : meme apres fermeture + retrait de `Desktop.Windows` + deux ticks `desktop.Update()` supplementaires, la fenetre (et donc tout son sous-arbre) reste reference — identifiee precisement par isolation experimentale (detachement cible via reflexion, teste avant/apres) : `MGWindow` cree en permanence, des son constructeur, un scope de ressources local via `EnsureResourceScope(UIResourceScope.Window)` (MGWindow.cs:1117, mecanisme totalement distinct de D1-D4) ; ce `MGResources` s'abonne a `Desktop.Resources.OnDefaultThemeChanged` (`MGResources.SetParent`/`Parent_OnDefaultThemeChanged`, MGResources.cs:85-120) et n'en desabonne jamais lorsque la fenetre se ferme ; chaine de retention verifiee : `Desktop.Resources -> (delegate cible) window.LocalResources -> (event backing field OnDefaultThemeChanged de LocalResources) lambda capturant `this` (MGElement.cs:524) -> window` ;
- **decision : documenter, ne pas corriger** — cette fuite releve bien du patron "abonnement element->desktop" du perimetre de ce plan, mais sa correction naive (desabonner au moment de `TryCloseWindow`, symetrique de ce qui a ete fait pour D1) romprait le patron "fermer n'est pas mourir" explicitement etabli par la section "Approche INTERDITE" de D1 : ce repo re-affiche couramment la MEME instance de `MGWindow` apres fermeture (`SampleBase.Show/Hide`, `BringToFront` via `Desktop.Windows.Remove`+`Add`) ; `Desktop.Windows` n'est qu'une `List<MGWindow>` (pas d'`ObservableCollection`), donc rien ne rattache automatiquement `MGResources.Parent` a la reouverture — desabonner au close laisserait une fenetre reouverte silencieusement orpheline de `Desktop.Resources` (plus de propagation de changement de theme). Corriger correctement exigerait de decider ou/quand re-attacher ce lien a la reouverture, une nouvelle decision de conception hors perimetre D1-D4 de cette tache ;
- le test `WindowResourcesParentChain_IsAKnownResidualRoot_NotFixedByThisPlan` pinne explicitement cette fuite residuelle (assert que la fenetre fermee reste `IsAlive`) ; le test principal `ClosingWindow_AllowsTextBoxNumericUpDownAndListBox_ToBeCollected_OnceKnownResidualRootIsDetached` neutralise cette unique racine connue via un helper de reflexion test-only (`DetachWindowLocalResourcesFromDesktop`, jamais appele en production) puis prouve que les 4 references (fenetre, textbox, numericupdown, listbox) deviennent collectables — isolant et verrouillant ainsi specifiquement que D1 (trackers), D2 (focus) et D3 (listbox EndUpdate) ne laissent plus aucune racine ;
- `Docs/event-handling-architecture.md` mis a jour : faiblesses 4, 5, 6 annotees **CORRIGE** avec renvoi vers ce plan et les fichiers de tests correspondants (faiblesse 5 mentionne desormais explicitement la fuite jumelle MGListBox et sa correction) ; nouveau paragraphe documentant la fuite residuelle `Desktop.Resources`/`MGWindow.LocalResources` (chaine de retention + justification du non-fix) juste apres la liste des 6 faiblesses confirmees ; section "Recommandations priorisees" mise a jour : points 1 et 2 marques **FAIT** avec renvoi aux decisions D1-D4 et a la regression de bout en bout, mention de la fuite residuelle documentee plutot que corrigee ;
- validation : `dotnet build MGUI.Tests.csproj --no-restore` vert (0 erreur) ; `dotnet build MGUI.Samples.csproj --no-restore` vert (0 erreur) ; `dotnet build MGUI.MiniGame.csproj --no-restore` vert (0 erreur) ; `dotnet test --filter "FullyQualifiedName~InputLifetimeRegressionTests"` : 2/2 verts ; `dotnet test --filter "Focus|Input"` : 360/360 verts ; suite complete : 1204 tests, 1201 reussis, 3 echecs (exactement les 3 rouges de la baseline tache 0 : `BackendProjectSplitTests.IntegrationProject_StripsLegacyRendererFiles`, `ToolingHooksTests.UIToolingService_ExposesSnapshotAndPreviewHooks`, `ControlTemplateInfrastructureTests.ListBox_Rehydrates_Theme_Dependent_Row_Chrome_On_Theme_Refresh`), 0 nouveau rouge, 0 ignores ;
- aucun code de production modifie pour cette tache (uniquement le nouveau fichier de tests et la documentation) ; aucune API publique ajoutee ou retiree.

Mise a jour post-plan (2026-09-01) : la racine residuelle documentee ci-dessus est desormais **corrigee**, via l'option "weak event cible sur ce seul point" (et non le detach/rattach au cycle open/close, impraticable : `Desktop.Windows` est une `List<MGWindow>` sans hook d'ajout, un scope detache sans theme local fait jeter `MGResources.DefaultTheme`, et `SetParent` ne re-propage pas un theme change survenu pendant le detachement). `MGResources.SetParent` s'abonne au parent via `WeakThemeChangedForwarder` (classe imbriquee de `MGResources`, reference faible vers le scope enfant, auto-desabonnement des forwarders morts). La philosophie "pas de weak events generalises" reste respectee : ce point d'abonnement est le seul a croiser deux domaines de duree de vie. Cote tests : `WindowResourcesParentChain_IsAKnownResidualRoot_NotFixedByThisPlan` est devenu `ClosedWindow_IsNotRootedByDesktopResourcesThemeSubscription` (assertion inversee : la fenetre fermee est collectable), le helper de reflexion `DetachWindowLocalResourcesFromDesktop` est supprime, et un nouveau test `ThemeChange_StillReachesWindow_WhileClosedAndAfterReopen` verrouille la propagation de theme sur une fenetre fermee puis re-affichee, a travers des GC. Voir `Docs/event-handling-architecture.md` (paragraphe "Fuite residuelle", statut CORRIGE).
