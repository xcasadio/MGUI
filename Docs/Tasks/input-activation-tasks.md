...
- Tests : nouveau fichier `MGUI.Tests/Input/WindowActivationOnClickTests.cs` (10 tests) couvrant chaque critere d'acceptation : clic sur fenetre en arriere-plan -> premier plan + resolution focus + `ActiveWindow`/`ActiveWindowChanged` ; clic sans cible de focus valide -> premier plan, focus inchange ; clic sur element focusable -> priorite au focus du clic sur la cible resolue ; `WindowFocusHistory` prioritaire sur `DefaultFocusElement` (`preferWindowDefault: false`) ; aucun effet sous fenetre modale (`PushModalWindow`) ; la branche `OverlayHost`/`AddOverlay` de la garde `MGDesktop.IsBlockedByModalOrOverlay` est desormais epinglee par un test dedie (`ClickActivation_HasNoEffect_WhileModalOverlayIsOpen`) ; l'investigation a etabli que cette garantie est en realite appliquee par trois gardes independantes et redondantes, chacune suffisante seule pour bloquer le clic : G1 `FocusInputPolicy.ShouldProcessWindowInputs` (`MGUI.Core/UI/FocusInputPolicy.cs:24-25`, consommee `MGDesktop.cs:1429`, appliquee `MGDesktop.cs:1437`) ; G2 `MGDesktop.IsBlockedByModalOrOverlay` elle-meme (`MGDesktop.cs:250-265`, branche overlay `:257-261`), appelee depuis le cablage `ActivatesOnClick` (`MGWindow.cs:1572`) ; G3 l'`OverlayWindow` plein ecran insere en premier dans `OrderedWindows` a chaque tick (`MGDesktop.cs:1419`), qui occlut ensuite toute fenetre traitee via `IsWindowOccludedAtMousePos` (`MGDesktop.cs:1447-1449`). Consequence : aucune mutation partielle (une seule garde retiree, ou meme deux des trois ensemble) ne fait rougir le test dedie -- trois experiences de mutation successives (G2 seule, G1 seule, G1+G2 ensemble) l'ont confirme, la garantie et le test restant intacts a chaque fois -- si bien que la couverture branche-par-branche de G2 initialement envisagee pour cette tache n'est pas atteignable sur le chemin du clic ; chevauchement bidirectionnel (pression occluse ne remonte pas, pression exposee remonte, assertions sur `desktop.Windows`) ; les quatre popups epingles a `ActivatesOnClick = false` ; clic sur item de dropdown `MGComboBox` ouvert ne reordonne pas `NestedWindows` (verifie juste apres la pression, avant que la selection ne ferme et retire le dropdown au relachement) ; clic dans un `MGContextMenu` ouvert (sur son separateur, point non focusable) ne perturbe ni `desktop.Windows` ni le focus existant. Nouveau fichier `MGUI.Tests/Input/FloatingDockWindowActivationTests.cs` (1 test) : epingle le comportement pre-existant de `MGFloatingDockWindow` (clic ramene au premier plan) apres la migration, via `MGDockHost`/`DockPanelNode` construits directement (sans passer par `DetachToFloating`). Les tests neufs sur la logique de resolution/precedence/garde-modale ont ete verifies discriminants par mutation manuelle (retrait temporaire de la garde de precedence du clic, inversion de `preferWindowDefault`, retrait de l'exclusion `Dropdown.ActivatesOnClick`) : chacun echoue comme attendu, puis le code de production a ete restaure a l'identique (diff verifie propre).
...
- Nouveau fichier `MGUI.Core/UI/ITextEntryHost.cs` : interface publique exactement conforme au texte de la tache (`bool IsReadonly { get; }`, `event EventHandler<bool> ReadonlyChanged`, `bool ShouldPreserveTextEntryKey(Keys key)`), namespace `MGUI.Core.UI`.
- `MGUI.Core/UI/MGTextBox.cs` : signatures reelles verifiees avant d'ecrire l'interface — `public bool IsReadonly { get; set; }` (`:935`) et `public event EventHandler<bool> ReadonlyChanged` (`:957`) correspondent exactement au texte de la tache, aucun blocage. `MGTextBox` declare desormais `: MGElement, ITextEntryHost`. Le membre `internal bool ShouldPreserveTextEntryKey(Keys key)` existant (`:69-70`) est conserve tel quel, sans aucune modification. Une implementation d'interface explicite est ajoutee juste en dessous : `bool ITextEntryHost.ShouldPreserveTextEntryKey(Keys key) => ShouldPreserveTextEntryKey(key);` — elle resout sans ambiguite vers le membre d'instance `internal` a un seul argument `Keys` (distinct de la surcharge statique 4-arguments de meme nom a `:51-52`). `IsReadonly`/`ReadonlyChanged` satisfont l'interface par implementation implicite (deja publics, aucun changement necessaire). La surface publique de `MGTextBox` ne s'agrandit pas : l'implementation explicite n'est visible qu'a travers une reference `ITextEntryHost`.
- `MGUI.Core/UI/MGDesktop.cs` : les 6 sites migres vers `is ITextEntryHost` / abonnement `ReadonlyChanged` via l'interface. Les numeros de ligne cites par la tache (`:545`, `:558`, `:1366`, `:953`, `:960`, `:985`) presentaient un decalage mineur avec le fichier reel (`:545`, `:558`, `:1384`, `:954`, `:961`, `:1001-1007`), du au deplacement de lignes par les taches 1 et 2 plus haut dans le meme fichier (meme phenomene deja documente dans le Resultat de la tache 2 pour `ResolveAutoFocusTarget`). Contenu verifie identique a chaque emplacement reel avant edition (memes variables, meme structure) : `ResolveSemanticInputMode` (`:545`), `ShouldCaptureGameplayInput` (`:558`), le meme test chemin `Update` brut (`:1384`, variable `isTextEntryFocused`), le setter de `FocusedKeyboardHandler` (desabonnement `Previous is ITextEntryHost` `:954`, abonnement `FocusedKeyboardHandler is ITextEntryHost` `:961`), et `TextBox_ReadonlyChanged` (`sender is ITextEntryHost` `:1003`). Aucune autre modification dans ce fichier.
- Tests : nouveau fichier `MGUI.Tests/Focus/ITextEntryHostThirdPartyTests.cs` (4 tests) avec un double tiers `ThirdPartyTextEntryHost : MGButton, ITextEntryHost` (hors `MGTextBox`, hors `MGUI.Core` text-editing) : focus + non-readonly -> `ShouldCaptureGameplayInput()` vrai ; focus + readonly -> `ShouldCaptureGameplayInput()` faux (prouve que la garde `!IsReadonly` fonctionne pour un type tiers dans les deux sens) ; passage a `IsReadonly = true` pendant que le double est focus -> `FocusedKeyboardHandler` redevient `null` (nettoyage via `ReadonlyChanged`, site setter+`TextBox_ReadonlyChanged`) ; passage a `IsReadonly = true` alors que le double n'est PAS focus -> aucun effet sur le focus existant (prouve le desabonnement correct). Verifie discriminants par mutation manuelle : remettre temporairement `is MGTextBox` au site `ShouldCaptureGameplayInput` fait echouer `ThirdPartyHost_FocusedAndNotReadonly_BlocksGameplayInput` ; remettre `is MGTextBox` au site d'abonnement du setter fait echouer `ThirdPartyHost_BecomingReadonlyWhileFocused_ClearsFocusedKeyboardHandler` ; code de production restaure a l'identique dans les deux cas (diff verifie propre) avant de continuer.
- `rg "is MGTextBox" MGUI.Core/UI/MGDesktop.cs` -> aucun resultat (verifie).
- Validation : `dotnet build MGUI.Tests/MGUI.Tests.csproj --no-restore` OK ; `dotnet build MGUI.Samples/MGUI.Samples.csproj --no-restore` OK ; `dotnet build MGUI.MiniGame/MGUI.MiniGame.csproj --no-restore` OK (couche semantique, requis pour les taches 3/4) ; `dotnet test MGUI.Tests/MGUI.Tests.csproj --no-build --filter "Focus|Input"` -> 434/434 verts (baseline tache 2 : 430 + 4 nouveaux) ; suite complete `dotnet test MGUI.Tests/MGUI.Tests.csproj --no-build` -> 1444/1444 verts (baseline 1440 + 4 nouveaux), aucun test rouge.
...
- `MGUI.Core/UI/Navigation/UIFocusNavigationService.cs` : les deux emplacements cites par la tache correspondaient exactement au code reel, verifie avant edition (aucun decalage de ligne cette fois, contrairement aux taches 1-3). `:216`, dans `TryDispatchNavigationAction(BaseKeyPressedEventArgs e)` : `focusedElement is MGTextBox` -> `focusedElement is ITextEntryHost` comme 3e argument (`isTextEntryFocused`) de `FocusInputPolicy.TryGetNavigationAction(...)`. `:454`, dans la methode privee `ShouldPreserveTextEntryKey(Keys? key)` : `focusedElement is MGTextBox focusedTextBox && focusedTextBox.ShouldPreserveTextEntryKey(keyValue)` -> `focusedElement is ITextEntryHost focusedTextEntryHost && focusedTextEntryHost.ShouldPreserveTextEntryKey(keyValue)`, qui n'appelle donc plus le membre `internal` de `MGTextBox` mais l'implementation d'interface explicite ajoutee par la tache 3. Les commentaires XML doc juste au-dessus de `:454` (qui citaient `MGTextBox`/`MGTextBox.ShouldPreserveTextEntryKey`) sont mis a jour vers `ITextEntryHost`/`ITextEntryHost.ShouldPreserveTextEntryKey` pour rester exacts ; le commentaire doc de la surcharge semantique a `:235` (exemple `MGTextBox`/`AcceptsTab`) est laisse tel quel, `AcceptsTab` etant une propriete specifique a `MGTextBox` et non a l'interface. Aucune autre ligne du fichier modifiee ; `rg "is MGTextBox" MGUI.Core/UI/Navigation/UIFocusNavigationService.cs` -> aucun resultat (verifie), de meme `rg "MGTextBox\."` -> aucun resultat.
- Tests : nouveau fichier `MGUI.Tests/Focus/UIFocusNavigationServiceThirdPartyTextEntryTests.cs` (3 tests), double tiers `ThirdPartyTextEntryHost : MGButton, ITextEntryHost` (hors `MGTextBox`) qui reserve une seule touche configurable via son constructeur. `RawNavigateLeft_WithFocusedThirdPartyHostReservingKey_PreservesKey_DoesNotDispatchNavigation` pilote le chemin clavier brut (`desktop.Update()` avec un `KeyboardState` pressant `Keys.Left`, cablage `HighPriorityKeyboardHandler.Pressed` -> `TryDispatchNavigationAction(BaseKeyPressedEventArgs)`, exercice du site `:216`) : le double focus et reservant `Keys.Left` garde le focus, un `MGButton` place a sa gauche n'est pas rejoint. `RawNavigateTab_WithFocusedThirdPartyHostNotReservingKey_MovesFocusToNextElement` est un test de controle (meme chemin brut, touche non reservee) qui prouve que la garde ne bloque pas tout le clavier des qu'un `ITextEntryHost` est focus. `SemanticNavigateLeft_WithFocusedThirdPartyHostReservingKey_PreservesKey_DoesNotDispatchNavigation` pilote le chemin semantique (`desktop.TryHandleInputAction` avec `InputAction.NavigateLeft`/`Keys.Left`, qui appelle directement la surcharge 3-arguments -> `ShouldPreserveTextEntryKey(key)`, exercice du site `:454` isole du site `:216`) avec le meme double et la meme assertion. Les 2 tests portant sur une touche reservee ont ete verifies discriminants par mutation manuelle : remettre temporairement `is MGTextBox` au site `:216` fait echouer uniquement le test du chemin brut (le test de controle et le test semantique restent verts) ; remettre `is MGTextBox` au site `:454` fait echouer a la fois le test du chemin brut (qui invoque aussi `ShouldPreserveTextEntryKey` en interne) et le test semantique ; code de production restaure a l'identique dans les deux cas (`git diff` verifie propre) avant de continuer.
- Validation : `dotnet build MGUI.Tests/MGUI.Tests.csproj --no-restore` OK ; `dotnet build MGUI.Samples/MGUI.Samples.csproj --no-restore` OK ; `dotnet build MGUI.MiniGame/MGUI.MiniGame.csproj --no-restore` OK ; `dotnet test MGUI.Tests/MGUI.Tests.csproj --no-build --filter "Focus|Input"` -> 437/437 verts (baseline tache 3 : 434 + 3 nouveaux) ; suite complete `dotnet test MGUI.Tests/MGUI.Tests.csproj --no-build` -> 1447/1447 verts (baseline 1444 + 3 nouveaux), aucun test rouge.

### ✅ 5. Point d'extension clavier de fenetre + depreciation

Le report par decision utilisateur (decision 6 de `Docs/input-window-activation-design.md`) est leve : cette tache a ete executee dans une session dediee, independamment des taches 1 a 4. La forme est celle deja decidee (decision 5 du meme document) : un handler "preview" par fenetre, pompe **avant** les enfants de la fenetre pendant `MGWindow.Update`, documente comme l'equivalent window-scoped du `HighPriorityKeyboardHandler` du desktop — pas un fallback pour les touches non consommees, ce qui serait du bubbling.
...
- Membre choisi : `MGWindow.PreviewKeyboardHandler` (`MGUI.Core/UI/MGWindow.cs`), remplacant documente de `WindowKeyboardHandler`.
- Proprietaire du handler : ni `MGWindow` (dont `MGElement` surcharge `IKeyboardHandlerHost.HasKeyboardFocus()` en `GetDesktop().FocusedKeyboardHandler == this`, ce qui rend `WindowKeyboardHandler` inerte) ni `MGDesktop`, mais un nouveau type dedie `WindowPreviewKeyboardHandlerHost` (nouveau fichier `MGUI.Core/UI/WindowPreviewKeyboardHandlerHost.cs`, `internal sealed class ... : IKeyboardHandlerHost { }`, aucune surcharge) qui conserve donc l'implementation par defaut de l'interface (`HasKeyboardFocus()` => `true`) — le meme mecanisme que celui deja utilise par `MGDesktop.HighPriorityKeyboardHandler`. Cree dans le constructeur de `MGWindow` via `InputTracker.Keyboard.CreateHandler(new WindowPreviewKeyboardHandlerHost(), null)`.
- Emplacement de la pompe : au debut du lambda `OnBeginUpdateContents` deja existant (`MGWindow.cs`, juste apres `ElementUpdateArgs UpdateArgs = e.UA.ChangeOffset(Origin);`), donc AVANT `UpdateContents` (mise a jour des enfants). Verifie dans `MGElement.Update` (`MGElement.cs:2587-2616`) : `OnBeginUpdateContents?.Invoke(...)` s'execute avant `UpdateContents(UAForContents)`, qui met a jour recursivement les descendants (chaque descendant pompe son propre `KeyboardHandler` a la toute fin de son propre `Update`, `MGElement.cs:2640`) ; `OnEndUpdateContents` (ou est pompe l'ancien `WindowKeyboardHandler`, inchange) s'execute lui APRES `UpdateContents`. Ce placement garantit donc que la semantique "preview" tient reellement : le handler de fenetre voit la touche du tick avant que le descendant focus (ex. un `MGTextBox`) ne la voie lui-meme.
- Depreciation : `WindowKeyboardHandler` porte desormais `[Obsolete("...")]` (message nommant `PreviewKeyboardHandler` comme remplacant et expliquant que `WindowKeyboardHandler` ne recoit jamais d'evenements car une `MGWindow` ne detient jamais elle-meme le focus clavier), comportement runtime strictement inchange (toujours cree et pompe, ne recoit toujours rien). Les 2 sites internes de `MGWindow.cs` qui creent (`:1395`, dans le constructeur) et pompent (`:1686`, dans `OnEndUpdateContents`) `WindowKeyboardHandler` sont chacun entoures d'un `#pragma warning disable CS0618` / `#pragma warning restore CS0618` avec un commentaire d'une ligne expliquant pourquoi ; aucune suppression globale. Un 3e avertissement CS0618 est apparu a la compilation, inattendu au depart : `nameof(WindowKeyboardHandler)` utilise DANS le message `[Obsolete]` du membre lui-meme declenchait aussi CS0618 ; corrige en remplacant les `nameof`/interpolation par une chaine litterale dans le message de l'attribut (pas de pragma supplementaire necessaire, la regle "suppression narrow aux 2 sites internes" reste donc respectee au sens strict).
- Regle de securite (guidance, pas un mecanisme force par le framework) : documentee dans le commentaire XML de `PreviewKeyboardHandler` — un abonne qui ne doit pas voler les touches d'edition doit verifier `MGDesktop.FocusedKeyboardHandler is ITextEntryHost` (et retourner tot si vrai) avant d'agir sur une touche ; exercee end-to-end par les tests.
- Tests : nouveau fichier `MGUI.Tests/Input/WindowPreviewKeyboardHandlerTests.cs` (3 tests), harnais `GraphTestRuntime`/`MGDesktop`/`MGWindow` avec un vrai clic souris dans un `MGTextBox` (focus + positionnement du caret, requis par le chemin d'insertion de `MGTextBox.HandleKeyPress`, cf. le meme harnais que `TextBoxNativeTabInputRegressionTests`). `PreviewKeyboardHandler_ReceivesPressedBeforeFocusedDescendant_AndCanConsumeItFirst` : preuve d'ordonnancement — l'abonne au niveau fenetre observe `e.IsHandled == false` a l'entree (preuve significative car `MGTextBox.KeyboardHandler.Pressed` marque toujours `IsHandled` une fois execute ; si la pompe avait ete placee apres les enfants, l'assertion aurait echoue), consomme un Ctrl+F via `e.SetHandledBy(window, false)`, et le `MGTextBox` ne tape jamais le caractere `f` (`Text` reste vide) puisque `KeyboardHandler.InvokeQueuedEvents` ne livre `Pressed` que si `!IsHandled`. `PreviewKeyboardHandler_ReproducesMotivatingPattern_...` : reproduit le scenario reel motivant (editeur externe) bout en bout — Ctrl+F gere au niveau fenetre meme avec le `MGTextBox` focus, PUIS une touche de frappe normale (`Keys.A`) qui n'est PAS volee car l'abonne retourne tot des que `Desktop.FocusedKeyboardHandler is MGTextBox` pour toute touche non reconnue comme raccourci, et le texte `"a"` apparait bien dans le `MGTextBox`. `WindowKeyboardHandler_StillReceivesNothing_WhileDescendantTextBoxIsFocused` : epingle que l'ancien membre `[Obsolete]` ne recoit toujours rien dans le meme scenario (abonnement entoure de `#pragma warning disable/restore CS0618` cote test egalement). Verifie discriminant par mutation manuelle : deplacement temporaire de la pompe de `OnBeginUpdateContents` vers `OnEndUpdateContents` (apres les enfants) fait echouer les 2 premiers tests (`previewSawCtrlF`/`ctrlFShortcutInvocations` restent a leur valeur initiale, la fenetre ne voit plus jamais la touche puisque le `MGTextBox` l'a deja consommee avant elle) ; code de production restaure a l'identique ensuite (`git status`/diff verifies propres) avant la validation finale.
- Validation : `dotnet build MGUI.Tests/MGUI.Tests.csproj --no-restore` OK, 0 erreur ; `dotnet build MGUI.Samples/MGUI.Samples.csproj --no-restore` OK, 0 erreur ; `dotnet build MGUI.MiniGame/MGUI.MiniGame.csproj --no-restore` OK, 0 erreur ; `dotnet test MGUI.Tests/MGUI.Tests.csproj --no-build --filter "Focus|Input"` -> baseline mesuree (nouveau fichier de test temporairement retire) 438/438 verts, puis 441/441 verts avec les 3 nouveaux tests ; suite complete `dotnet test MGUI.Tests/MGUI.Tests.csproj --no-build` -> 1451/1451 verts (baseline enonce de la tache : 1448 + 3 nouveaux), aucun test rouge.
- `WindowKeyboardHandler` reste conserve car consomme par une application externe (CasaEngine.Editor), conformement a la decision utilisateur de ne pas le supprimer.
