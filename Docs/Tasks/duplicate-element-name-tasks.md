...
- P2. **Deux causes distinguees dans le message.** Les deux elements portent leur `XamlSourcePosition` dans `MGElement.Metadata` quand ils viennent d'un document (`UIToolingService.XamlSourcePositionMetadataKey`). Si les deux positions ont le **meme `Ordinal`**, il s'agit d'une seule declaration clonee par un template ; sinon, de deux declarations. Le message le dit :
  - meme ordinal (formulation livree, revue comprise) : `Duplicate element name 'ItemLabel'. A MGTextBlock already carries that name in this window, and both elements carry the same source position: line 5, column 18 of 'repro.xaml'. A name declared once and reaching more than one element is what a template does, applying it to every element it generates. Remove the name from the template content, or make the template generate a single element. A name may only identify one element of a window.` La position partagee est rapportee comme l'indice qu'elle est ; le template est nomme comme cause, pas affirme (voir R2)
...
- `UIToolingService.TryResolveXamlElementType(localName, out dtoType)` expose deja le predicat « ce nom local est un DTO derivant d'`Element` » (`UIToolingService.cs:82`).
...
### ✅ T5. Suite complete, verification independante et rapport
...
Note du 25 septembre 2026 : la cause a ete identifiee depuis, et ce n'est pas la compilation JIT etagee. A la fin de son marquage, un GC d'arriere-plan lance par un autre test annule le contexte d'allocation du thread de mesure sans retirer du compteur la partie non utilisee de ce contexte (jusqu'a ~8 Ko). Le test ouvre maintenant sa fenetre par `AllocationWindow.Start()` (`MGUI.Tests/AllocationWindow.cs`, 9081461), et la reference `LayoutTransitionTests.cs:944-946` ne pointe plus sur ce commentaire.
...
- apres T4, une verification d'issue sur l'etat final : **CONFIRMED** sur les six criteres (A1 a A6), chaque garde-fou controle par mutation de la source livree. Trois avis non bloquants, tous corriges ici : trois references `fichier:ligne` fausses dans le plan (dont une lue sur `xaml-editor`, ou la methode ne se trouve pas a la meme ligne que sur `develop`), une description de test devenue caduque apres `eefc9c1`, et le fait qu'une exception re-enveloppee par l'hote garde son code mais perd sa position, desormais dit dans `Docs/editor-architecture.md`.
