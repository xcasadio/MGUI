...
- ✅ (clos par note : ces brushes ne sont pas dessines ; regle conservee par le test FillBrushSlots_AreTickedOrDocumentedExclusions ; a reactiver si un jour ils sont dessines) `MGGraphView` (`GridLineBrush`, `MajorGridLineBrush`, `EdgeBrush`) : seule la couleur est extraite par `MGGraphSurfaceCanvas.ResolveBrushColor`. Si ces brushes sont un jour dessines, surcharger `GetFillBrushes()`. Tout nouvel emplacement `IFillBrush` / `VisualStateFillBrush` de controle doit etre classe (dessine directement / proxy / modele) et le test `FillBrushSlots_AreTickedOrDocumentedExclusions` mis a jour.
- ✅ (commit 2f48243) `MGNineSliceFillBrush` : decomposer les neuf patchs sur la geometrie arrondie via `DrawTexturedTriangleList` (repli rectangle commente dans `MGNineSliceFillBrush.Draw(..., MGBoxShape, MGBoxGeometry)`).
- ✅ (commit c5e5efd) `MGHighlightFillBrush` : construire les masques d'exclusion sur le mesh au lieu de la soustraction de rectangles (repli commente dans l'overload shape-aware).
- ✅ (commit 8157658) `MGHighlightBorderBrush`, modes `Progress` et `Scan` : parametrer l'animation le long du contour arrondi (`OuterContour` / `InnerContour`) au lieu du perimetre rectangulaire (repli commente dans l'overload shape-aware).
- ✅ (commit cd2f96b) `MGTextureFillBrush` en mode `Tile` sur un sous-rectangle d'atlas : un sampler Wrap repeterait tout l'atlas ; tuiler par quads textures clippes a la forme ou par un atlas dedie (repli commente).
...
- ✅ (clos par note : aucun profil ne le justifie a ce jour ; critere d activation conserve : allocation par frame visible au profil) `MGTexturedBorderBrush` arrondi : les tableaux par region sont alloues a chaque frame ; mettre en cache par geometrie et transformations si le profil le justifie.
