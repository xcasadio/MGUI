...
- **Cause de fermeture et fermeture par code** (ADR-0018) : les arguments de `PanelClosing` sont toujours un `DockPanelClosingEventArgs` (le type de l'evenement ne change pas, on caste) ; `ClosingFloatingWindow` nomme la fenetre flottante quand c'est la fermeture de cette fenetre **entiere** qui leve l'evenement, et vaut null sur tous les autres chemins. Un abonne qui refuse pour poser une question asynchrone ferme ensuite le panneau par `MGDockHost.ClosePanel(DockPanelNode)` : meme effet que la fermeture utilisateur une fois `PanelClosing` passe (panneau ancre, auto-masque, ou dans une fenetre flottante suivie par l'hote ; `PanelRemoved` leve ; une fenetre flottante videe se ferme), sans lever `PanelClosing`. `RemovePanel` ne suffit pas : un panneau detache dans une fenetre flottante quitte le registre de l'hote et `RemovePanel` ne le voit plus. Pour une fenetre entiere, l'abonne rappelle `ClosingFloatingWindow.TryCloseWindow()` apres sa reponse : les panneaux restants sont redemandes, dans l'ordre.
