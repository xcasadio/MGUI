...
- **`MGDockHost`** (`MGSingleContentHost`) : racine du systeme ; `LayoutModel` -> reconstruction de l'arbre visuel ; events `PanelAdded` / `PanelRemoved` / `PanelClosing` / `ActivePanelChanged` / `DockLayoutChanged` ; pile maximize/restore ; docking par proximite (`ProximityDockingEnabled`, `ProximityBandWidth`) ; parts `PART_PreviewOverlay`, `PART_DropIndicators`, `PART_LeftAutoHideStrip` / `PART_RightAutoHideStrip` / `PART_TopAutoHideStrip` / `PART_BottomAutoHideStrip`, `PART_AutoHideDrawer`.
- **`PanelClosing`** (`EventHandler<CancelEventArgs<DockPanelNode>>`, T4.5) : leve AVANT tout retrait qu'un geste **utilisateur** decide, un panneau a la fois, pour laisser un abonne l'annuler (`args.Cancel = true`) — l'hote de l'editeur s'en sert pour demander s'il faut enregistrer un ecran modifie avant de le fermer. Chemins couverts : la croix d'un onglet, "Close Others"/"Close All" (un panneau refuse reste, les autres du meme lot se ferment quand meme, un par un) ; l'onglet d'une fenetre flottante (modele ou autonome) ; la fermeture du tiroir d'auto-hide ; la fermeture d'une fenetre flottante **entiere** (`MGWindow.TryCloseWindow`, sa croix ou du code applicatif) — l'hote s'abonne alors au `WindowClosing` annulable de la fenetre flottante et leve `PanelClosing` pour chacun de ses panneaux dans l'ordre, annulant la fermeture de la fenetre au premier refus ; les panneaux deja demandes avant ce refus, dans la meme passe, gardent ce que leur abonne a fait (pas d'annulation retroactive). PAS leve pour un retrait programmatique : `RemovePanel` ou `CloseFloatingWindow` appeles directement par le code applicatif. Sample : la case "Unsaved changes" du panneau Scratchpad dans `DockingDemo`.
- **Cause de fermeture et fermeture par code** (ADR-0018) : les arguments de `PanelClosing` sont toujours un `DockPanelClosingEventArgs` (le type de l'evenement ne change pas, on caste) ; `ClosingFloatingWindow` nomme la fenetre flottante quand c'est la fermeture de cette fenetre **entiere** qui leve l'evenement, et vaut null sur tous les autres chemins. Un abonne qui refuse pour poser une question asynchrone ferme ensuite le panneau par `MGDockHost.ClosePanel(DockPanelNode)` : meme effet que la fermeture utilisateur une fois `PanelClosing` passe (panneau ancre, auto-masque, ou dans une fenetre flottante suivie par l'hote ; `PanelRemoved` leve ; une fenetre flottante videe se ferme), sans lever `PanelClosing`. `ClosePanel` trouve le panneau par le modele (`LayoutModel`) et les fenetres flottantes suivies, jamais par le registre, comme les chemins utilisateur : `RemovePanel` ne suffit pas, puisqu'un panneau detache dans une fenetre flottante quitte le registre, et qu'un panneau ancre directement dans le modele par `DockOperation.DockAsTab` sans `RegisterPanel` (ainsi l'editeur de CasaEngine ouvre ses documents) n'y est jamais entre. Pour une fenetre entiere, l'abonne rappelle `ClosingFloatingWindow.TryCloseWindow()` apres sa reponse : les panneaux restants sont redemandes, dans l'ordre.
...
- **Controles** : `MGDockSplitContainer` + `MGDockSplitterBar` (`DockSplitSizing`), `MGDockTabGroup` + `MGDockTabItem`, `MGDockAutoHideStrip` + `MGDockAutoHideDrawer`, `MGFloatingDockWindow` (fenetre `MGWindow` flottante rattachee a son `OwnerHost`). `MGDockSplitContainer`, `MGDockTabGroup` et `MGDockAutoHideDrawer` sont des `MGContentHost` : chaque contenu de panneau qu'ils prennent ou rendent (reconstruction de l'arbre, changement d'onglet, flottement, tiroir) est annonce a la chaine des hotes de contenu, donc a l'index des noms de la fenetre, et une fenetre flottante rend son contenu quand l'hote cesse de la suivre (ADR-0015). Un `Name` pose dans le contenu d'un panneau est ainsi resolu par la fenetre tant que le panneau est affiche, et refuse s'il y est deja.
